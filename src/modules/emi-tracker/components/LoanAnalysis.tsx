...
const CustomTooltip = ({ active, payload, label, currencySymbol, numberFormat }: CustomTooltipProps) => {
...
const DistributionLegend = (props: { payload?: ChartEntry[] }) => {
  ...
  return (
    <div className="flex flex-wrap justify-center gap-4 mt-4">
      {payload?.map((entry, index: number) => (
        <div key={index} className="flex items-center gap-1.5 text-[10px] font-bold text-zinc-400">
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.color }} />
          {entry.name}
        </div>
      ))}
...
const CostLegend = (props: { payload?: ChartEntry[]; totalPrincipal: number; totalInterest: number }) => {
        ...
        {payload?.map((entry, index: number) => (
          <div key={index} className="flex flex-col items-center gap-1">
            ...
            <span className="text-xs font-black text-zinc-300 italic">
              {total > 0 ? (( (entry.payload?.value ?? entry.value) / total) * 100).toFixed(1) : "0.0"}%
            </span>
          </div>
        ))}
      </div>
...
export default function LoanAnalysis({
  ...
  const distributionData = useMemo(() => [
    { name: "Principal Paid", value: metrics.paidPrincipal, color: "#10b981" },
    { name: "Principal Remaining", value: metrics.remainingPrincipal, color: "#3f3f46" },
    { name: "Interest Paid", value: metrics.paidInterest, color: "#f43f5e" },
    { name: "Interest Remaining", value: metrics.remainingInterest, color: "#71717a" },
  ], [metrics]);

  const totalCostData = useMemo(() => [
    { name: "Total Principal", value: metrics.totalPrincipal, color: "#10b981" },
    { name: "Total Interest", value: metrics.totalInterest, color: "#f43f5e" },
  ], [metrics]);
  ...
  const historyData = useMemo(() => {
    ...
    const step = schedule.rows.length > 120 ? 6 : (schedule.rows.length > 48 ? 3 : 1);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
          <div className="relative z-10 mb-6">
            <h3 className="text-sm font-bold text-zinc-300">Loan Status Distribution</h3>
            <p className="text-[10px] text-zinc-500 mt-1 uppercase tracking-widest font-black italic">Paid vs. Remaining Lifecycle</p>
          </div>
          <div className="h-[300px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Legend verticalAlign="bottom" height={36} content={<DistributionLegend />} />
              </PieChart>
            </ResponsiveContainer>
        ...
        <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
          <div className="relative z-10 mb-6">
            <h3 className="text-sm font-bold text-zinc-300">Projected Total Cost</h3>
            <p className="text-[10px] text-zinc-500 mt-1 uppercase tracking-widest font-black italic">Structural Overhead Ratio</p>
          </div>
          <div className="h-[300px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip content={<CustomTooltip currencySymbol={currencySymbol} numberFormat={numberFormat} />} />
                <Legend verticalAlign="bottom" height={36} content={<CostLegend totalPrincipal={metrics.totalPrincipal} totalInterest={metrics.totalInterest} />} />
              </PieChart>
            </ResponsiveContainer>
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
      ...
      <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 shadow-xl">
        <div className="mb-6">
          <h3 className="text-sm font-bold text-zinc-300">Cumulative Liability Breakdown</h3>
          <p className="text-[10px] text-zinc-500 mt-1 uppercase tracking-widest font-black italic">Principal vs. Interest accumulation trend</p>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={historyData}>
              ...
              <YAxis 
                ...
                tickLine={false} 
                tick={{ fill: '#71717a', fontSize: 10 }}
                tickFormatter={(v) => formatMoney(v, currencySymbol, 0, numberFormat).replace(/[₹$€]/, '')}
              />
              <Tooltip content={<CustomTooltip currencySymbol={currencySymbol} numberFormat={numberFormat} />} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-2xl bg-zinc-950/20 border border-zinc-800/50 flex flex-col gap-2">
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Efficiency Ratio</span>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-black text-zinc-100 italic">{metrics.totalInterest > 0 ? (metrics.totalPrincipal / metrics.totalInterest).toFixed(2) : "∞"}</span>
              <span className="text-[10px] font-bold text-zinc-500 text-xs text-nowrap">P:I Factor</span>
            </div>
            <p className="text-[10px] text-zinc-500 italic mt-1 leading-relaxed">Factor above 1.0 means principal is higher than total interest expense.</p>
        </div>
        <div className="p-6 rounded-2xl bg-zinc-950/20 border border-zinc-800/50 flex flex-col gap-2">
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Payoff Completion</span>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-black text-accent italic">{(metrics.paidPrincipal / metrics.totalPrincipal * 100).toFixed(1)}%</span>
              <span className="text-[10px] font-bold text-zinc-500 text-xs uppercase">Principal</span>
            </div>
            <p className="text-[10px] text-zinc-500 italic mt-1 leading-relaxed">Percentage of initial loan amount cleared from the lender&apos;s book.</p>
        </div>
        <div className="p-6 rounded-2xl bg-zinc-950/20 border border-zinc-800/50 flex flex-col gap-2">
