...
import { Plus, Trash2, FileText, UploadCloud, Link as LinkIcon } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="bg-zinc-950/40 border border-zinc-800/80 rounded-2xl p-5">
        <h3 className="text-sm font-bold text-zinc-300 mb-4">Attach Documents</h3>
        <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2">
            <label className="block text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-1.5 px-0.5">Document Title</label>
            <input
          ...
          <div className="lg:col-span-2">
            <label className="block text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-1.5 px-0.5">Upload File</label>
            <div className="flex items-center gap-2">
              <label className="flex-1 flex items-center justify-center gap-2 border-2 border-dashed border-zinc-800 hover:border-accent/40 hover:bg-zinc-900/50 rounded-xl p-2.5 cursor-pointer transition-all group">
                ...
                {isUploading && <div className="w-3 h-3 border-2 border-accent/20 border-t-accent rounded-full animate-spin" />}
              </label>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {documents.length === 0 ? (
          <div className="md:col-span-2 bg-zinc-950/20 border border-zinc-800/50 rounded-2xl p-8 text-center shadow-lg">
            <p className="text-zinc-500 text-sm italic font-medium">No documents attached yet.</p>
          </div>
        ) : (
          documents.map((doc, idx) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-zinc-100 line-clamp-1">{doc.title}</p>
                  <p className="text-[10px] text-zinc-500 mt-1 font-black uppercase tracking-widest">{doc.type.replace('_', ' ')}</p>
                </div>
              </div>
