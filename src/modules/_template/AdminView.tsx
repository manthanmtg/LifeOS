...
export default function TemplateAdminView() {
  // 1. Module Settings
  const { settings, saving: settingsSaving } =
    useModuleSettings<TemplateSettings>("templateSettings", DEFAULT_SETTINGS);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      return (
        item.payload.name.toLowerCase().includes(query) ||
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="text-zinc-400 mt-1">
              A playground for standardized features and accessibility.
            </p>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 animate-fade-in-up">
          ...
          >
            <div className="space-y-1.5">
              <label htmlFor="form-name" className="text-xs text-zinc-500">
                Name
              </label>
              <input
                id="form-name"
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredItems.map((item) => (
              <article
                key={item._id}
                className="group bg-zinc-900 border border-zinc-800 rounded-2xl p-5 hover:border-zinc-700 transition-colors"
              >
