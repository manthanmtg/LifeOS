...
function getStreak(completions: { date: string; count: number }[]): {
  current: number;
  longest: number;
} {
...
export default function HabitsAdminView() {
  ...
  } = useModuleSettings("habitSettings", HABIT_DEFAULTS);
  ...
  const [targetCount, setTargetCount] = useState(
    settings.defaultTarget.toString(),
  );
  ...
  const resetForm = () => {
    setName("");
    setDescription("");
    setFrequency(settings.defaultFrequency);
    setTargetCount(settings.defaultTarget.toString());
    setColor(COLORS[0]);
    setEditingId(null);
    setFormError("");
    setShowForm(false);
  ...
  const handleEdit = (h: Habit) => {
    setName(h.payload.name);
    setDescription(h.payload.description || "");
    setFrequency(h.payload.frequency);
    setTargetCount(h.payload.target_count.toString());
    setColor(h.payload.color);
    setEditingId(h._id);
    setShowForm(true);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  updateSettings({ defaultFrequency: e.target.value })
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  updateSettings({ heatmapMonths: parseInt(e.target.value) })
                }
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-zinc-50 focus:outline-none focus:ring-2 focus:ring-accent/40"
              >
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-2 text-sm text-zinc-400 cursor-pointer pb-1">
                <input
                  ...
                  onChange={(e) =>
                    updateSettings({ weekStartMon: e.target.checked })
                  }
        ...
        <div className="space-y-4">
          {habits.map((habit) => {
            ...
            return (
              <div
                key={habit._id}
                className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 group"
              >
                        ...
                        {Array.from({ length: 7 }, (_, dayIdx) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "w-3 h-3 rounded-[2px] transition-all hover:scale-125",
