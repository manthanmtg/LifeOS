...
export default function BlogAdminView() {
  ...
  const stats = useMemo(() => {
    ...
    const drafts = posts.filter(
      (post) => post.payload.status === "draft",
    ).length;
    const archived = posts.filter(
      (post) => post.payload.status === "archived",
    ).length;
      ...
      <div className="relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        <div>
          ...
          <p className="text-zinc-400 mt-1">
            Rich markdown authoring with live preview, keyboard shortcuts, and
            autosave.
          </p>
        </div>
        ...
        <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 md:p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1">
                ...
                {lastSavedAt
                  ? ` • Last saved ${new Date(lastSavedAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}`
                  : ""}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              {slugConflict && (
                <p className="text-xs text-danger mt-1">Slug already exists.</p>
              )}
            </div>
          </div>
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    value={draft.seoDesc}
                    onChange={(e) =>
                      setDraft((prev) => ({ ...prev, seoDesc: e.target.value }))
                    }
                    rows={3}
                  ...
                  {post.payload.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {post.payload.tags.slice(0, 4).map((tag) => (
                        <span
                          key={tag}
                          className="px-2 py-0.5 text-[11px] rounded-full border border-zinc-700 bg-zinc-800 text-zinc-400"
                        >
                          {tag}
                        </span>
                      ))}
      ...
      {!showEditor && (
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4 text-sm text-zinc-400">
          <p className="inline-flex items-center gap-2">
            ...
            Rich editor includes markdown toolbar, live preview, and autosave
            (local for new drafts, server for saved posts).
          </p>
        </div>
      )}
    </div>
