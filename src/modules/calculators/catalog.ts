...
function computeSipFutureValue(
...
function computeTaxFromSlabs(
  ...
  for (const slab of slabs) {
    ...
    const taxableAtThisRate = Math.max(
      0,
      Math.min(taxableIncome, upper) - previousLimit,
    );
...
export const CALCULATOR_CATEGORIES: CalculatorCategory[] = [
  {
    ...
    description:
      "Plan long-term wealth goals with SIP, lumpsum, and retirement cashflow tools.",
  },
  {
    id: "debt",
    name: "Loans & Debt",
    description:
      "Understand EMIs, prepayments, and refinancing impact before loan decisions.",
  },
  {
    id: "tax",
    name: "Tax & Benefits",
    description:
      "Estimate taxes and long-term benefit payouts for better planning.",
  },
  {
    id: "returns",
    name: "Returns Analysis",
    description:
      "Evaluate growth quality with CAGR, XIRR, and inflation-adjusted views.",
  },
  {
    id: "conversion",
    name: "Conversions",
    description:
      "Convert units quickly across length, weight, temperature, data, and more.",
  },
  {
    id: "utilities",
    name: "Home & Utilities",
    description:
      "Practical calculators for household planning, capacity, and energy decisions.",
  },
];
...
export const CALCULATOR_DEFINITIONS: CalculatorDefinition[] = [
      ...
      return {
        ...
        metrics: [
          ...
          },
          {
            ...
            value: `${formatNumber(result.invested > 0 ? result.value / result.invested : 0, 2)}x`,
          },
        ],
      };
    },
  },
  {
    ...
    compute: (values) => {
      ...
      const lastYearSip =
        startingSip * safePow(1 + stepUp, Math.max(0, years - 1));
      ...
      return {
        ...
        metrics: [
          ...
          {
            label: "Gain over invested",
            value: formatPercent(invested > 0 ? (gain / invested) * 100 : 0, 1),
            tone: "good",
          },
        ],
      };
      ...
      return {
        ...
        metrics: [
          ...
          },
          {
            ...
            value: `${formatNumber(principal > 0 ? futureValue / principal : 0, 2)}x`,
          },
        ],
      };
    },
    ...
    compute: (values) => {
      ...
      if (requiredFutureFromSip > 0) {
        if (monthlyRate === 0) {
          ...
          const factor =
            ((safePow(1 + monthlyRate, months) - 1) / monthlyRate) *
            (1 + monthlyRate);
        ...
        metrics: [
          {
            ...
            value: formatMoney(futureOfCurrent),
          },
          {
            label: "Monthly SIP needed",
            value: formatMoney(requiredMonthly),
            tone: requiredMonthly > 0 ? "warn" : "good",
          },
          {
            label: "Total future contribution",
            value: formatMoney(totalMonthlyInvested),
          },
        ],
      };
    },
  },
  {
    ...
    compute: (values) => {
      ...
      return {
        primaryLabel:
          corpus > 0 ? "Corpus after projection" : "Corpus depletion",
        primaryValue:
          corpus > 0
            ? formatMoney(corpus)
            : `${formatNumber(yearsLasted, 1)} years`,
  ...
  },
  {
    ...
    categoryId: "debt",
    description:
      "Estimate tenure reduction and interest saved from extra payments.",
    inputs: [
      {
        key: "principal",
        label: "Loan amount",
      ...
      {
        key: "startAfterMonths",
        label: "Start prepayment after",
        kind: "number",
        defaultValue: "12",
        min: 0,
        max: 360,
    ...
    compute: (values) => {
      ...
      const startAfter = Math.max(
        0,
        Math.round(getNumber(values, "startAfterMonths", 12)),
      );
      const months = Math.max(1, Math.round(years * MONTHS_PER_YEAR));
      ...
      const interestSaved = Math.max(
        0,
        base.interestPaid - optimized.interestPaid,
      );
        ...
        metrics: [
          {
            ...
            value: `${formatNumber(base.months / 12, 1)} years`,
          },
          {
            label: "With prepayment",
            value: `${formatNumber(optimized.months / 12, 1)} years`,
            tone: "good",
          },
        ...
        metrics: [
          {
            ...
            value: formatMoney(existing + proposed),
          },
          {
            label: "Disposable after EMI",
            value: formatMoney(Math.max(0, income - existing - proposed)),
          },
  ...
  },
  {
    ...
    compute: (values) => {
      ...
      return {
        ...
        secondaryValue:
          savings >= 0
            ? "Transfer looks beneficial"
            : "Transfer may not be worth it",
        metrics: [
          { label: "Current EMI", value: formatMoney(emiCurrent) },
          {
            label: "New EMI",
            value: formatMoney(emiNew),
            tone: emiNew <= emiCurrent ? "good" : "warn",
          },
        ...
        metrics: [
          {
            ...
            tone: oldTax <= newTax ? "good" : "warn",
          },
          {
            label: "Tax (New)",
            value: formatMoney(newTax),
            tone: newTax <= oldTax ? "good" : "warn",
          },
          {
            label: "Effective tax rate",
            value: formatPercent(
              income > 0 ? (Math.min(oldTax, newTax) / income) * 100 : 0,
              2,
            ),
          },
        ],
  ...
  },
  {
    ...
    compute: (values) => {
      ...
      return {
        ...
        secondaryValue: eligible
          ? "Eligible under 5+ year rule"
          : "Becomes eligible after 5 years",
        metrics: [
  ...
  },
  {
    ...
    compute: (values) => {
      ...
      const annual = getNumber(values, "annualReturn", 10);
      const years = getNumber(values, "years", 25);
      const annuityPercent = clamp(
        getNumber(values, "annuityPercent", 40),
        0,
        100,
      );
        ...
        metrics: [
          {
            ...
            value: formatPercent(((end - start) / start) * 100, 2),
          },
          {
            label: "Wealth multiple",
            value: `${formatNumber(end / start, 2)}x`,
          },
          {
            label: "Annualized growth",
            value: formatPercent(cagr, 2),
  ...
  },
  {
    ...
    inputs: [
      {
        ...
        defaultValue:
          "2022-01-01, -100000\n2022-07-15, -50000\n2023-12-01, -25000\n2025-03-01, 230000",
        helper:
          "Use negative amounts for investments and positive for withdrawals/redemption.",
    ...
    compute: (values) => {
      ...
      const result: CalculatorResult = {
        primaryLabel: "XIRR",
        primaryValue:
          xirr === null ? "Insufficient data" : formatPercent(xirr * 100, 2),
        ...
        metrics: [
          ...
          {
            label: "Net gain",
            value: formatMoney(inflow - outflow),
            tone: inflow >= outflow ? "good" : "warn",
          },
        ],
        notes:
          xirr === null
      ...
      return {
        ...
        metrics: [
          { label: "Nominal future value", value: formatMoney(futureNominal) },
          {
            ...
            value: formatMoney(realFutureValue),
          },
          {
            label: "Purchasing-power impact",
            value: formatMoney(futureCost - amount),
            tone: "warn",
          },
        ],
      };
  ...
  },
  {
    ...
    compute: (values) => {
      ...
      return {
        ...
        metrics: [
          {
            label: "Rule-72 estimate",
            value: `${formatNumber(estimateYears, 2)} years`,
          },
          {
            label: "Exact formula",
            value: `${formatNumber(exactYears, 2)} years`,
          },
          {
            label: "Difference",
            value: `${formatNumber(Math.abs(exactYears - estimateYears), 2)} years`,
          },
        ],
      };
    },
  },
  {
    ...
    description:
      "Estimate annual savings and payback from rooftop solar setup.",
    inputs: [
      {
        key: "monthlyUnits",
    ...
    compute: (values) => {
      ...
      for (let year = 0; year < 25; year += 1) {
        const annualGeneration =
          monthlyGeneration * 12 * safePow(1 - degradation, year);
      ...
      return {
        ...
        secondaryValue: Number.isFinite(payback)
          ? `Simple payback ${formatNumber(payback, 1)} years`
          : "Payback not achieved with current inputs",
        metrics: [
          {
            label: "Monthly solar generation",
            value: `${formatNumber(monthlyGeneration, 0)} kWh`,
          },
          {
            label: "Monthly bill offset",
            value: `${formatNumber(monthlyOffset, 0)} kWh`,
          },
          {
            label: "25-year net savings",
            value: formatMoney(lifetimeSavings),
            tone: lifetimeSavings >= 0 ? "good" : "warn",
          },
        ],
      };
    },
  },
  {
    ...
    compute: (values) => {
      ...
      const validAspect =
        Number.isFinite(aspectW) &&
        Number.isFinite(aspectH) &&
        aspectW > 0 &&
        aspectH > 0;
      const widthFactor = validAspect
        ? Math.sqrt(aspectW * aspectW + aspectH * aspectH) / aspectW
        : 1.147;
      ...
      return {
        ...
        metrics: [
          {
            label: "Room size",
            value: `${formatNumber(roomWidthFt, 1)} x ${formatNumber(roomLengthFt, 1)} ft`,
          },
          {
            label: "Seating distance",
            value: `${formatNumber(seatingFt, 1)} ft`,
          },
          {
            label: "Wall-constrained max",
            value: `${formatNumber(maxWallDiagonal, 0)} in`,
          },
        ],
      };
    },
  },
  {
    ...
    compute: (values) => {
      ...
      const occupants = Math.max(
        1,
        Math.round(getNumber(values, "occupants", 3)),
      );
      ...
      return {
        ...
        metrics: [
          { label: "Room area", value: `${formatNumber(area, 0)} sq ft` },
          {
            ...
            value: `${formatNumber(tonnage, 2)} ton`,
          },
  ...
  },
  {
    ...
    description:
      "Convert between millimeter, centimeter, meter, kilometer, inch, foot, yard, and mile.",
    inputs: [
      {
        key: "value",
        label: "Value",
  ...
  },
  {
    ...
    categoryId: "conversion",
    description:
      "Convert between milligram, gram, kilogram, tonne, ounce, and pound.",
    inputs: [
      {
        key: "value",
        label: "Value",
    ...
    compute: (values) => {
      ...
      return {
        ...
        metrics: [
          ...
          {
            label: "Freezing point delta",
            value: `${formatNumber(celsius - 0, 2)} C`,
          },
          {
            label: "Boiling point delta",
            value: `${formatNumber(100 - celsius, 2)} C`,
          },
        ],
      };
    },
  ...
  },
  {
    ...
    categoryId: "conversion",
    description:
      "Convert between sq mm, sq cm, sq m, sq km, sq ft, sq yd, acre, and hectare.",
    inputs: [
      {
        key: "value",
        label: "Value",
        ...
        metrics: [
          {
            ...
            value: `${formatNumber(sqMeters, 6)} sq m`,
          },
          { label: "From unit", value: from },
          { label: "To unit", value: to },
        ],
  ...
  },
  {
    ...
    categoryId: "conversion",
    description:
      "Convert between milliliter, liter, cubic meter, cubic inch, cubic foot, cup, and gallon.",
    inputs: [
      {
        key: "value",
        label: "Value",
  ...
  },
  {
    ...
    categoryId: "conversion",
    description:
      "Convert between meter/sec, kilometer/hour, mile/hour, foot/sec, and knot.",
    inputs: [
      {
        key: "value",
        label: "Value",
