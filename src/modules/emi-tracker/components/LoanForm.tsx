...
export default function LoanForm({
  ...
  const [category, setCategory] = useState(editLoan?.payload.category || settings.categories[0]);
  const [currency, setCurrency] = useState(editLoan?.payload.currency || settings.defaultCurrency);
  const [principal, setPrincipal] = useState(editLoan?.payload.principal || 1000000);
  const [tenure, setTenure] = useState(editLoan?.payload.tenure_months || 120);
  const [interestRate, setInterestRate] = useState(editLoan?.payload.annual_interest_rate || 9.5);
  const [emi, setEmi] = useState(editLoan?.payload.monthly_emi || 0);
  const [startDate, setStartDate] = useState(editLoan?.payload.start_date ? toDateInputValue(editLoan.payload.start_date) : toDateInputValue(new Date().toISOString()));
  const [dueDay, setDueDay] = useState(editLoan?.payload.due_day_of_month || settings.defaultDueDayOfMonth);
  const [recastStrategy, setRecastStrategy] = useState(editLoan?.payload.recast_strategy || settings.defaultRecastStrategy);
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div className="group">
              <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block group-focus-within:text-accent transition-colors">Loan Narrative</label>
              <input 
                required 
                value={title} 
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Currency</label>
                <input 
                  value={currency} 
            ...
            <div className="space-y-4">
              <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block">Principal Amount</label>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block italic">Interest Rate (%)</label>
              ...
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block italic">Tenure (Months)</label>
                <input 
                  type="number" 
                  value={tenure} 
            ...
            <div className="space-y-4">
              <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block">Monthly Installment (EMI)</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">First Payment</label>
              ...
              <div>
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 block">Due Day</label>
                <input 
        ...
        <div className="pt-8 border-t border-zinc-800/50 flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">Strategy</p>
                <div className="flex items-center gap-4 mt-1">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input type="radio" name="recast" checked={recastStrategy === 'keep_emi_adjust_tenure'} onChange={() => setRecastStrategy('keep_emi_adjust_tenure')} className="accent-accent" />
                    <span className="text-xs font-bold text-zinc-400 group-hover:text-zinc-200">Adjust Tenure</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input type="radio" name="recast" checked={recastStrategy === 'keep_tenure_adjust_emi'} onChange={() => setRecastStrategy('keep_tenure_adjust_emi')} className="accent-accent" />
                    <span className="text-xs font-bold text-zinc-400 group-hover:text-zinc-200">Adjust EMI</span>
                  </label>
                </div>
              </div>
           </div>
