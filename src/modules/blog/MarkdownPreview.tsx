  ...
  const components: Components = {
    ...
    code: ({ className: codeClassName, children, ...props }) => {
      ...
      if (inline) {
        return (
          <code
            className="bg-zinc-800 border border-zinc-700 rounded px-1.5 py-0.5 text-[0.9em] text-zinc-100"
            {...props}
          >
            {children}
          </code>
      ...
      return (
        <pre className="bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 overflow-x-auto">
          <code
            className={cn("font-mono text-sm text-zinc-100", codeClassName)}
            {...props}
          >
            {children}
          </code>
    ...
    blockquote: ({ children }) => (
      <blockquote className="border-l-2 border-accent/40 pl-4 text-zinc-300/90 italic">
        {children}
      </blockquote>
    ),
  };
