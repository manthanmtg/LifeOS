        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((item) => (
            ...
            >
              ...
              {/* Series progress */}
              {(item.payload.type === "series" ||
                item.payload.type === "anime") &&
                item.payload.current_season && (
                  <p className="text-[10px] text-zinc-500">
                    S{item.payload.current_season}
                    {item.payload.current_episode
                      ? ` · E${item.payload.current_episode}`
                      : ""}
                    {item.payload.total_seasons
                      ? ` / ${item.payload.total_seasons} seasons`
                      : ""}
                  </p>
                )}
