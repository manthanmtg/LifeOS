...
export function SpreadsheetTab({
  ...
  areas,
  records,
  schedulePeriods: externalPeriods,
  setActiveCropId,
  onReorderPeriods,
  onRefresh,
}: SpreadsheetTabProps) {
  ...
  useEffect(() => {
    setLocalPeriods([...externalPeriods]);
  }, [externalPeriods]);
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center py-12 shadow-sm">
        ...
        <p className="text-zinc-500 mt-1">
          Please configure your crops and areas in the Crop Settings tab first.
        </p>
      </div>
    );
  }
  ...
  const handleSavePeriod = async (period: string) => {
    ...
    try {
      ...
      const url = periodData._id
        ? `/api/content/${periodData._id}`
        : "/api/content";
      const reqBody = periodData._id
        ? { payload }
        : { module_type: "crop_history", is_public: false, payload };
  ...
  const handleAddPeriod = () => {
    ...
    if (val && !schedulePeriods.includes(val)) {
      ...
      setLocalData((prev: LocalData) => ({
        ...prev,
        [val]: { source_data: {}, summary_data: {}, notes: "" },
      }));
    }
    setNewPeriodValue("");
  ...
  const handleDeletePeriod = async (period: string) => {
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        if (periodData?._id) {
          try {
            const res = await fetch(`/api/content/${periodData._id}`, {
              method: "DELETE",
            });
  ...
  const getFormulaContext = (period: string): FormulaContext => {
    ...
    for (const aId of areaIds) {
      ...
      for (const f of activeCrop.sourceFields) {
        areaValues[aId][f.id] =
          Number(periodData.source_data?.[aId]?.[f.id]) || 0;
      }
    }
              ...
              {areas.map((area: AreaDef) => (
                ...
                >
                  ...
                  {schedulePeriods.map((period: string, pIdx: number) => (
                    <td
                      key={`${area.id}-${period}`}
                      className="px-0 py-0 border-r border-zinc-800 align-top bg-zinc-950/20 min-w-[150px] md:min-w-[180px]"
                    >
                      <div className="flex flex-col h-full divide-y divide-zinc-800/50">
                        {activeCrop.sourceFields.map((f: FieldDef) => {
                          const val =
                            Number(
                              localData[period]?.source_data?.[area.id]?.[f.id],
                            ) || 0;
                          const prevPeriod =
                            pIdx > 0 ? schedulePeriods[pIdx - 1] : null;
                          const prevVal = prevPeriod
                            ? Number(
                                localData[prevPeriod]?.source_data?.[area.id]?.[
                                  f.id
                                ],
                              ) || 0
                            : 0;
                          return (
                            <div
                              key={f.id}
                              className="flex justify-between items-center px-4 py-2 hover:bg-zinc-800"
                            >
                              <span
                                className="text-xs text-zinc-500 w-16 md:w-24 shrink-0 truncate"
              ...
              {areas.length > 1 && activeCrop.sourceFields.length > 0 && (
                <tr className="bg-zinc-800/40 border-t-2 border-zinc-700">
                  ...
                  {schedulePeriods.map((period: string, pIdx: number) => (
                    <td
                      key={`totals-${period}`}
                      className="px-0 py-0 border-r border-zinc-800 align-top min-w-[150px] md:min-w-[180px]"
                    >
                      <div className="flex flex-col h-full divide-y divide-zinc-700/50">
                        {activeCrop.sourceFields.map((f: FieldDef) => {
                          ...
                          const prevTotal = prevPeriod
                            ? getSourceFieldTotal(prevPeriod, f.id)
                            : 0;
                          return (
                            <div
                              key={f.id}
                              className="flex justify-between items-center px-4 py-2 bg-zinc-800/30"
                            >
                              <span className="text-xs text-zinc-400 w-16 md:w-24 shrink-0 truncate font-medium">
                                {f.name}
                              </span>
                              <div className="flex items-center gap-1">
                    ...
                    >
                      <div className="flex flex-col h-full divide-y divide-zinc-800/50">
                        {activeCrop.summaryFields.map((f: FieldDef) => {
                          const val =
                            Number(localData[period]?.summary_data?.[f.id]) ||
                            0;
                          const prevPeriod =
                            pIdx > 0 ? schedulePeriods[pIdx - 1] : null;
                          const prevVal = prevPeriod
                            ? Number(
                                localData[prevPeriod]?.summary_data?.[f.id],
                              ) || 0
                            : 0;
                          return (
                            <div
              ...
              {activeCrop.calculatedFields.length > 0 && (
                <tr className="bg-accent/5 hover:bg-accent/10 transition-colors border-t-2 border-accent/20">
                  ...
                  {schedulePeriods.map((period: string, pIdx: number) => {
                    ...
                    const prevPeriod =
                      pIdx > 0 ? schedulePeriods[pIdx - 1] : null;
                    const prevResults = prevPeriod
                      ? getCalcResults(prevPeriod)
                      : {};
                    return (
                      <td
                        key={`calc-${period}`}
                      ...
                      >
                        <div className="flex flex-col h-full divide-y divide-zinc-800/50">
                          {activeCrop.calculatedFields.map(
                            (f: CalcFieldDef) => {
