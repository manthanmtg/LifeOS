...
export default function ReadingAdminView() {
  ...
  } = useModuleSettings("readingSettings", READING_DEFAULTS);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data.error || `Failed to ${editingId ? "update" : "save"} item`,
        );
  ...
  const filtered = useMemo(() => {
    ...
    return [...items]
      ...
      .sort((a, b) => {
        const prioritySort =
          (priorityOrder[a.payload.priority] ?? 1) -
          (priorityOrder[b.payload.priority] ?? 1);
    ...
    <div className="animate-fade-in-up space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                onChange={(event) =>
                  updateSettings({ defaultPriority: event.target.value })
                }
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map((item) => {
            const PriorityIcon =
              PRIORITY_ICONS[item.payload.priority] || ArrowRightCircle;
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-label={
                    item.payload.is_read ? "Mark as unread" : "Mark as read"
                  }
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-zinc-500 flex-wrap">
                    {item.payload.source_domain && (
                      <span>{item.payload.source_domain}</span>
                    )}
