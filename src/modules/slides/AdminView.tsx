...
export default function SlidesAdminView() {
  ...
  const toggleVisibility = async (item: DeckItem) => {
    ...
    try {
      const res = await fetch(`/api/content/${item._id}`, {
        ...
        body: JSON.stringify({
          ...
          payload: { ...item.payload, visibility: newVisibility },
        }),
      });
  ...
  const filtered = useMemo(() => {
    ...
    return sortedItems.filter((item) => {
      if (formatFilter !== "all" && item.payload.format !== formatFilter)
        return false;
      if (
        visibilityFilter !== "all" &&
        item.payload.visibility !== visibilityFilter
      )
        return false;
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 animate-fade-in-up space-y-4">
          ...
          >
            ...
            <div className="md:col-span-2">
              <label
                htmlFor="deck-url"
                className="block text-xs text-zinc-500 mb-1.5 flex items-center gap-1.5"
              >
      ...
      {viewingDeck && (
        <Suspense fallback={null}>
          <SlideViewer
            ...
            startIndex={Math.max(
              0,
              filtered.findIndex((d) => d._id === viewingDeck._id),
            )}
