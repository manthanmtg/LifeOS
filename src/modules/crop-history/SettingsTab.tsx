...
export function SettingsTab({
  ...
  const handleAddArea = () => {
    ...
    updateSettings({
      sources: [...(settings.sources || []), { id: newId, name: areaName }],
    });
  ...
  const handleDeleteArea = (id: string) => {
    setConfirmState({
      ...
      title: "Delete Area",
      description:
        "Delete this area? Historical data is kept but the area won't appear for new entries.",
      onConfirm: () => {
        updateSettings({
          sources: settings.sources.filter((s: AreaDef) => s.id !== id),
        });
  ...
  const handleSaveCrop = () => {
    ...
    const newId =
      cropForm.id || cropForm.name.toLowerCase().replace(/[^a-z0-9]/g, "_");
    const existingIndex = (settings.crops || []).findIndex(
      (c: CropConfig) => c.id === newId,
    );
    const newCrops = [...(settings.crops || [])];
    const cropData = { ...cropForm, id: newId };
    if (existingIndex >= 0) {
      newCrops[existingIndex] = cropData;
    } else {
      newCrops.push(cropData);
    }
  ...
  const handleDeleteCrop = (id: string) => {
    setConfirmState({
      ...
      onConfirm: () => {
        updateSettings({
          crops: settings.crops.filter((c: CropConfig) => c.id !== id),
        });
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-sm">
        <div className="flex justify-between items-center mb-2">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5">
              Locations or plots where your crops grow. Data is tracked
              separately per area.
            </p>
          </div>
          <button
            onClick={() => setIsEditingArea(true)}
            className="text-xs bg-zinc-800 hover:bg-zinc-700 text-zinc-200 px-3 py-1.5 rounded-lg flex items-center gap-2 transition-colors"
          >
        ...
        {isEditingArea && (
          <div className="my-4 bg-zinc-950 p-4 rounded-xl border border-zinc-800 flex items-end gap-3">
            ...
            <button
              ...
              className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 p-2 rounded-lg"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mt-4">
          {(settings.sources || []).map((src: AreaDef) => (
            <div
              key={src.id}
              className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3 flex justify-between items-center group"
            >
              <div>
              ...
              <button
                ...
                className="text-zinc-500 hover:text-danger p-1.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity"
              >
          ...
          {!settings.sources?.length && !isEditingArea && (
            <p className="text-sm text-zinc-500 col-span-full py-4 text-center border-dashed border-2 border-zinc-800 rounded-xl">
              No areas configured. Add your first area to get started.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-sm">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5">
              Define crop types with their fields and calculation formulas.
            </p>
          </div>
          <button
            onClick={() => {
          ...
          <div className="mb-6 bg-zinc-950 border border-zinc-800 rounded-xl p-5 animate-fade-in-down">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-zinc-100 font-semibold text-lg">
                {cropForm.id ? "Edit Crop Type" : "New Crop Type"}
              </h3>
              <button
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const newName = e.target.value;
                    const isNew = !settings.crops.find(
                      (c) => c.id === cropForm.id,
                    );
                    setCropForm({
                      ...cropForm,
            ...
            <div className="space-y-6 border-t border-zinc-800 pt-5">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 mb-3">
                  e.g. &quot;Undried&quot; weight in kg, &quot;OT&quot;
                  percentage. Each area will have its own value.
                </p>
                  ...
                  {cropForm.sourceFields.map((f: FieldDef, i: number) => (
                    <FieldChip
                      ...
                      onRemove={() =>
                        setCropForm({
                          ...cropForm,
                          sourceFields: cropForm.sourceFields.filter(
                            (_: FieldDef, idx: number) => idx !== i,
                          ),
                        })
                      }
                      onUpdate={(updated) => {
                        const next = [...cropForm.sourceFields];
                        next[i] = updated;
                        setCropForm({ ...cropForm, sourceFields: next });
                      }}
                    />
                  ))}
                </div>
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 mb-3">
                  e.g. &quot;Average Price&quot; in ₹ per 50kg bag — shared
                  across all areas for that period.
                </p>
                <div className="flex flex-wrap gap-2 mb-3">
                  {cropForm.summaryFields.map((f: FieldDef, i: number) => (
                    <FieldChip
                      ...
                      onRemove={() =>
                        setCropForm({
                          ...cropForm,
                          summaryFields: cropForm.summaryFields.filter(
                            (_: FieldDef, idx: number) => idx !== i,
                          ),
                        })
                      }
                      onUpdate={(updated) => {
                        const next = [...cropForm.summaryFields];
                        next[i] = updated;
                        setCropForm({ ...cropForm, summaryFields: next });
                      }}
                    />
                  ))}
                </div>
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 mb-3">
                  e.g. bag size (120kg), conversion factors. Use the constant
                  name directly in formulas.
                </p>
                <div className="flex flex-wrap gap-2 mb-3">
                  {(cropForm.constants || []).map(
                      ...
                      >
                        ...
                        <X
                          ...
                          onClick={() =>
                            setCropForm({
                              ...cropForm,
                              constants: (cropForm.constants || []).filter(
                                (_: ConstantDef, idx: number) => idx !== i,
                              ),
                            })
                          }
                        />
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 mb-3">
                  Use functions like{" "}
                  <code className="text-success bg-zinc-800 px-1 rounded">
                    SUM(undried)
                  </code>{" "}
                  or{" "}
                  <code className="text-warning bg-zinc-800 px-1 rounded">
                    WEIGHTED_AVG(ot, undried)
                  </code>
                  . Click chips to build your formula.
                </p>

                {/* Existing calculated fields */}
                {cropForm.calculatedFields.length > 0 && (
                  <div className="flex flex-col gap-2 mb-4">
                    {cropForm.calculatedFields.map(
                      (f: CalcFieldDef, i: number) => (
                        <div
                          key={i}
                          ...
                          <button
                            onClick={() =>
                              setCropForm({
                                ...cropForm,
                                calculatedFields:
                                  cropForm.calculatedFields.filter(
                                    (_: CalcFieldDef, idx: number) => idx !== i,
                                  ),
                              })
                            }
                ...
                <div className="bg-zinc-900 border border-zinc-700 border-dashed rounded-xl p-4 space-y-3">
                  <div className="grid grid-cols-[1fr_auto_auto] gap-3">
                    ...
                    <div>
                      ...
                      <input
                        value={newCalcUnit}
                        onChange={(e) => setNewCalcUnit(e.target.value)}
              ...
              {(cropForm.sourceFields.length > 0 ||
                cropForm.calculatedFields.length > 0) && (
                <div>
                  <div className="flex items-baseline gap-2 mb-1">
                    <label className="text-sm text-zinc-300 font-medium">
                  ...
                  <p className="text-xs text-zinc-500 mb-3">
                    Select the primary yield field and revenue field so KPIs and
                    charts display correctly.
                  </p>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs text-zinc-500 mb-1 block">
                        Primary Yield (per-area field)
                      </label>
                      <select
                        ...
                        onChange={(e) =>
                          setCropForm({
                            ...cropForm,
                            analyticsConfig: {
                              ...cropForm.analyticsConfig,
                              yieldFieldId: e.target.value || undefined,
                            },
                          })
                        }
                        className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100"
                    ...
                    <div>
                      <label className="text-xs text-zinc-500 mb-1 block">
                        Revenue / Income (calculated field)
                      </label>
                      <select
                        ...
                        onChange={(e) =>
                          setCropForm({
                            ...
                            analyticsConfig: {
                              ...cropForm.analyticsConfig,
                              revenueFieldId: e.target.value || undefined,
                            },
                          })
                        }
                        className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100"
                      >
                        <option value="">
                          Auto-detect (first currency field)
                        </option>
            ...
            <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-zinc-800">
              <button
                onClick={() => setIsEditingCrop(false)}
                className="px-5 py-2.5 rounded-lg text-sm text-zinc-400 hover:bg-zinc-800 transition-colors"
              >
                Cancel
              </button>
              <button
        ...
        <div className="space-y-4">
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <span className="text-xs text-zinc-500 bg-zinc-800 px-2 py-0.5 rounded-full mt-1 inline-block">
                    {crop.scheduleType}
                  </span>
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className="p-2 text-zinc-400 hover:bg-zinc-800 rounded-lg hover:text-zinc-200"
                  >
                    <Edit3 className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleDeleteCrop(crop.id)}
                    className="p-2 text-zinc-400 hover:bg-zinc-800 rounded-lg hover:text-danger"
                  >
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {(crop.summaryFields || []).map((f: FieldDef) => (
                      <div
                        key={f.id}
                        className="flex justify-between items-center bg-success/5 border border-success/10 px-2.5 py-1.5 rounded-md"
                      >
          ...
          {!settings.crops?.length && !isEditingCrop && (
            <p className="text-sm text-zinc-500 py-8 text-center border-dashed border-2 border-zinc-800 rounded-xl">
              No crops configured. Click &quot;Create New Crop Type&quot; to
              begin.
            </p>
          )}
        </div>
