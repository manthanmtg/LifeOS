...
async function fetchOpenAIUsage(
  ...
  do {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `OpenAI Usage API ${res.status}: ${errText.slice(0, 200)}`,
      );
    }
  ...
  do {
    ...
    if (!res.ok) {
      console.warn(
        `OpenAI Cost API returned ${res.status}, proceeding without cost data`,
      );
      break;
    }
    ...
    for (const bucket of data.data || []) {
      ...
      for (const result of bucket.results || []) {
        ...
        const key = `${dayKey}::${model}`;
        costByDayModel[key] =
          (costByDayModel[key] || 0) + (result.amount?.value || 0);
      }
    }
...
async function fetchAnthropicUsage(
  ...
  do {
    ...
    if (!res.ok) {
      const errText = await res.text();
      throw new Error(
        `Anthropic Usage API ${res.status}: ${errText.slice(0, 200)}`,
      );
    }
    ...
    for (const bucket of buckets) {
      ...
      const dayKey =
        extractDayKey(bucket.starting_at) ||
      ...
      for (const item of results) {
        ...
        usageByDayModel[key].cache_write +=
  ...
  do {
    ...
    if (!res.ok) {
      console.warn(
        `Anthropic Cost API returned ${res.status}, proceeding without cost data`,
      );
      break;
    }
    ...
    for (const bucket of data.data || []) {
      ...
      if (costResults.length > 0) {
        for (const r of costResults) {
          ...
          costByDay[dayKey] =
            (costByDay[dayKey] || 0) + (isNaN(costValue) ? 0 : costValue / 100);
        }
      } else {
