...
export default function ShoppingListAdminView() {
  ...
  const updateListPayload = async (
    listId: string,
    updatedPayload: ShoppingListPayload,
  ) => {
    // Optimistic update
    setLists((prev) =>
      prev.map((l) =>
        l._id === listId ? { ...l, payload: updatedPayload } : l,
      ),
    );
  ...
  const toggleCompleteList = (list: ShoppingListDocument) => {
    ...
    showToast(
      isNowCompleted ? "List marked as completed" : "List restored to active",
      "success",
    );
  ...
  const filteredLists = lists
    .filter(
      (l) =>
        l.payload && l.payload.is_completed === (activeTab === "completed"),
    )
    .filter((l) =>
  ...
  const toBuyItems =
    selectedList?.payload?.items?.filter((i) => !i.purchased) || [];
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <form
              ...
              className="lg:col-span-2 relative group flex items-center gap-3"
            >
            ...
            {loading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-32 bg-zinc-900/50 border border-zinc-800/50 rounded-2xl animate-pulse"
                  />
                ))}
              </div>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-zinc-500 font-medium">
                          {(list.payload?.items || []).length} items •{" "}
                          {
              ...
              <div className="flex flex-col items-center justify-center py-24 text-zinc-500 bg-zinc-900/20 border-2 border-dashed border-zinc-900 rounded-3xl">
                ...
                <p className="text-zinc-600 text-sm mt-1">
                  {activeTab === "active"
                    ...
                    : "Lists you complete will appear here"}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-zinc-50 font-outfit">
                  {selectedList?.payload.title}
                </h2>
                <p className="text-xs text-zinc-500 font-medium">
          ...
          {!selectedList?.payload.is_completed && (
            <form
              onSubmit={(e) => handleAddItem(selectedList!._id, e)}
                ...
                <div className="absolute top-full left-0 right-0 mt-2 p-3 bg-zinc-900 border border-zinc-800 rounded-2xl z-10 shadow-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="text-xs">
                    ...
                    <p className="text-zinc-500">
                      ...
                      {parseSmartEntry(quickAddItem).quantity && (
                        <span className="text-accent mx-1">
                          {parseSmartEntry(quickAddItem).quantity}
                        </span>
                      )}
                      {parseSmartEntry(quickAddItem).unit && (
          ...
          <div className="flex-1 overflow-y-auto space-y-8 pr-2">
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between px-1">
                ...
                <span className="text-[10px] font-bold text-zinc-600 px-2 py-0.5 bg-zinc-900 border border-zinc-800 rounded-lg">
                  {toBuyItems.length}
                </span>
              </div>
                  ...
                  <AnimatePresence mode="popLayout">
                    {toBuyItems.map((item) => (
                      ...
                      >
                        <button
                          onClick={() =>
                            toggleItemPurchased(selectedList!._id, item.id)
                          }
            ...
            <section className="space-y-4">
              ...
              >
                <div className="flex items-center gap-2">
                  <h3 className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em] group-hover/sec:text-zinc-400 transition-colors">
                    Purchased
                  </h3>
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      clearPurchased(selectedList!._id);
                    }}
                    className="text-[10px] font-bold text-zinc-700 hover:text-danger uppercase tracking-wider transition-colors"
                  >
                    Clear
                  </button>
                  <span className="text-[10px] font-bold text-zinc-700 px-2 py-0.5 bg-zinc-900 border border-zinc-800 rounded-lg">
