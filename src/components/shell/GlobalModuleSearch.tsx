    ...
    <>
      {parts.map((part, index) => (
        <span
          key={`${part.text}-${index}`}
          className={
            part.highlighted ? "bg-accent/20 text-accent rounded-sm" : undefined
          }
        >
          {part.text}
        </span>
      ...
      <div className="relative">
        <div
          className={cn(
            ...
            showResults
              ? "border-zinc-700 shadow-2xl shadow-black/40 ring-1 ring-zinc-700/50"
              : "border-zinc-800/50",
        ...
        >
          ...
          {!isSidebar && (
            <div className="flex flex-wrap items-center gap-2 border-t border-zinc-900 px-4 py-3 text-xs text-zinc-500 sm:px-5">
              <span className="uppercase tracking-[0.22em] text-zinc-600">
                Global module search
              </span>
              <span className="hidden sm:inline">
                Real-time fuzzy filtering across {modules.length} modules
              </span>
            </div>
          )}
        </div>
                          ...
                          <>
                            <p className="mt-1 line-clamp-2 text-xs text-zinc-500">
                              <HighlightedText
                                ...
                                matches={result.descriptionMatches}
                              />
                            </p>
