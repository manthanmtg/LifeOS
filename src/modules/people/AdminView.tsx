...
function isUpcomingBirthday(birthday?: string): boolean {
  ...
  return (
    (thisYear >= now && thisYear <= thirtyDaysLater) ||
...
export default function PeopleAdminView() {
  ...
  const [interactionType, setInteractionType] =
    useState<InteractionType>("other");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: Record<string, unknown> = {
      ...
      social_links: formSocialLinks.filter(
        ...
        setViewingPerson((prev) =>
          prev
            ? {
                ...
                payload: {
                  ...prev.payload,
                  is_favorite: !prev.payload.is_favorite,
                },
              }
            : null,
        );
  ...
  const handleLogInteraction = async () => {
    ...
    const updatedInteractions = [
      ...(viewingPerson.payload.interactions || []),
      newInteraction,
    ];
  ...
  const stats = useMemo(() => {
    ...
    const upcomingBirthdays = people.filter((p) =>
      isUpcomingBirthday(p.payload.birthday),
    ).length;
            ...
            <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    ...
                    {isUpcomingBirthday(person.payload.birthday) && (
                      <span className="text-xs px-2.5 py-1 rounded-full border border-warning/25 bg-warning/10 text-warning flex items-center gap-1">
                        <Cake className="w-3 h-3" /> Birthday in{" "}
                        {daysUntilBirthday(person.payload.birthday!)} days
                      </span>
                    )}
                  </div>
                </div>
              ...
              {(person.payload.company || person.payload.role) && (
                <p className="text-sm text-zinc-400 mt-2 flex items-center gap-1.5">
                  ...
                  {person.payload.role}
                  {person.payload.role && person.payload.company ? " at " : ""}
              ...
              <div className="flex flex-wrap gap-4 mt-4 text-sm">
                ...
                {person.payload.birthday && (
                  <span className="flex items-center gap-1.5 text-zinc-400">
                    ...
                    {formatDate(person.payload.birthday)}
                  </span>
                )}
              </div>
              ...
              {days !== null && (
                <div
                  className={cn(
                    "inline-flex items-center gap-1.5 mt-3 text-xs px-2.5 py-1 rounded-full",
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="text-zinc-400 mt-1">
                Your personal CRM for managing contacts and interactions.
              </p>
            </div>
            <button
              onClick={() => {
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFormRelationship(e.target.value as Relationship)
                        }
                        className={INPUT_CLASS}
                        disabled={isSubmitting}
                      >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <Heart
                        className="w-4 h-4"
                        fill={formIsFavorite ? "currentColor" : "none"}
                      />
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2 shrink-0">
                    {formError && (
                      <span className="text-danger text-xs mr-auto">
                        {formError}
                      </span>
                    )}
                    <button
