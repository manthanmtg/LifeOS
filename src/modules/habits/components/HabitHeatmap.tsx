      ...
      <div className="min-w-max pb-1">
        ...
        <div className="h-4 relative mb-1 ml-8">
          {monthLabels.map((m) => (
            <span
              key={`${m.label}-${m.col}`}
              className="text-xs text-zinc-600 font-bold uppercase tracking-tighter absolute whitespace-nowrap"
              style={{
                left: `${m.col * 15}px`,
              }}
            >
          ...
          <div className="flex flex-col gap-[3px] mr-2 pt-0.5">
            {dayLabels.map((label, i) => (
              <div key={i} className="h-3 flex items-center justify-end">
                <span className="text-xs text-zinc-600 w-6 text-right font-medium">
                  {label}
                </span>
              </div>
            ))}
                  ...
                  return (
                    <motion.button
                      ...
                      className={cn(
                        ...
                        isToday &&
                          "ring-1 ring-zinc-500 ring-offset-1 ring-offset-zinc-950",
                        isCompleted &&
                          "shadow-[0_0_8px_currentColor] hover:opacity-100",
                      )}
                      style={{
