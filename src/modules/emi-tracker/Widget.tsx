...
type ScheduleRowLite = {
  index: number;
  due_date: string;
  closing_balance: number;
};

function computeScheduleLite(
...
function getOutstandingAsOf(
  schedule: ScheduleRowLite[],
  startPrincipal: number,
  asOf: Date,
) {
  if (schedule.length === 0)
    return {
      outstanding: startPrincipal,
      nextDue: null as ScheduleRowLite | null,
    };
  const next =
...
export default function EmiTrackerWidget() {
  const { settings } = useModuleSettings<EmiTrackerWidgetSettings>(
    "emiTrackerSettings",
    { defaultCurrency: "INR", roundingDecimals: 2, numberFormat: "western" },
  );
  ...
  const summary = useMemo(() => {
    ...
    for (const l of active) {
      const processingFee =
        (l.payload.processing_fee_amount ?? 0) +
        (l.payload.processing_fee_percent
          ? (l.payload.processing_fee_percent / 100) * l.payload.principal
          : 0);
      ...
      const { outstanding, nextDue } = getOutstandingAsOf(
        sched,
        startPrincipal,
        now,
      );
      ...
      if (nextDue) {
        if (
          !nearest ||
          new Date(nextDue.due_date).getTime() < new Date(nearest.due).getTime()
        ) {
      ...
      <div className="py-2 space-y-4">
        <div>
              ...
              {summary.outstandingByCurrency[0] ? (
                <>
                  <span className="text-zinc-500 mr-1 text-2xl font-medium">
                    {CURR_SYM[summary.outstandingByCurrency[0].currency] ||
                      summary.outstandingByCurrency[0].currency}
                  </span>
                  {formatNumber(
                    summary.outstandingByCurrency[0].amount,
                    format,
                  )}
                </>
              ) : (
          ...
          <p className="text-xs text-zinc-500 mt-1 font-medium italic">
            total outstanding debt · {summary.activeCount} active
          </p>
          ...
          {summary.outstandingByCurrency.length > 1 && (
            <div className="mt-3 space-y-1.5">
              {summary.outstandingByCurrency.map((c) => (
                <div
                  key={c.currency}
                  className="flex items-center justify-between px-3 py-1.5 bg-zinc-950/40 border border-zinc-800/60 rounded-lg"
                >
                  ...
                  <span className="text-xs font-bold text-zinc-300">
                    {formatCurrency(
                      c.amount,
                      CURR_SYM[c.currency] || c.currency,
                      format,
                    )}
                  </span>
                </div>
