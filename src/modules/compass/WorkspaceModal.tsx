...
export default function WorkspaceModal({ task, onClose, onUpdate }: Props) {
  ...
  const [selectedSubtaskIndex, setSelectedSubtaskIndex] = useState<
    number | null
  >(null);
  const [isEditingSubtaskDescription, setIsEditingSubtaskDescription] =
    useState(false);
  const [editingSubtaskCommentIndex, setEditingSubtaskCommentIndex] = useState<
    number | null
  >(null);
  ...
  const removeChecklist = (index: number) => {
    updateField(
      "checklist",
      payload.checklist.filter((_, i) => i !== index),
    );
  ...
  const removeTag = (tag: string) => {
    updateField(
      "category_tags",
      payload.category_tags.filter((t) => t !== tag),
    );
  };
  ...
  const removeComment = (index: number) => {
    updateField(
      "comments",
      payload.comments.filter((_, i) => i !== index),
    );
  };
                ...
                {isEditingDescription ? (
                  ...
                  >
                    <MarkdownRenderer
                      content={
                        payload.description ||
                        "_No description provided. Click to add one..._"
                      }
                      className="prose-sm"
                    />
                    ...
                    <div key={i} className="flex gap-4 group/comment">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        {editingCommentIndex === i ? (
                          ...
                          <div className="relative bg-zinc-900/20 border border-zinc-800/40 group-hover/comment:border-zinc-800 rounded-2xl p-4 cursor-text transition-all">
                            <MarkdownRenderer
          ...
          <div className="w-80 bg-zinc-900/30 overflow-y-auto p-6 space-y-8">
            ...
            <div>
              ...
              <input
                type="date"
                value={
                  payload.target_date ? payload.target_date.slice(0, 10) : ""
                }
                onChange={(e) =>
                  updateField(
                    "target_date",
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {payload.category_tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1.5 px-2.5 py-1 bg-zinc-800 text-zinc-300 rounded-md text-xs border border-zinc-700 group"
                  >
                    {tag}
                    <button
            ...
            <div>
              ...
              <div className="space-y-2 mb-3">
                {payload.links.map((link, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between group bg-zinc-900 border border-zinc-800 px-3 py-2 rounded-lg"
                  >
                    <a
                      href={link.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-sm text-accent hover:underline truncate pr-4"
                    >
                      {link.label}
                    </a>
                    <button
          ...
          <div className="bg-zinc-950 border border-zinc-800 shadow-2xl rounded-3xl w-full max-w-3xl h-[80vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-10">
              ...
              <div>
                ...
                {isEditingSubtaskDescription ? (
                  <textarea
                    ...
                    value={
                      payload.checklist[selectedSubtaskIndex].description || ""
                    }
                    onChange={(e) => {
                      const cl = [...payload.checklist];
              ...
              <div>
                      ...
                      <div className="flex flex-col gap-2">
                        <button
                          onClick={() => {
                            const cl = [...payload.checklist];
                            const comments =
                              cl[selectedSubtaskIndex].comments || [];
                            cl[selectedSubtaskIndex].comments = [
                              {
                ...
                <div className="space-y-6">
                  {payload.checklist[selectedSubtaskIndex].comments?.map(
                    (comment, i) => (
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          {editingSubtaskCommentIndex === i ? (
                            <textarea
                              ...
                              onChange={(e) => {
                                const cl = [...payload.checklist];
                                cl[selectedSubtaskIndex].comments[i].text =
                                  e.target.value;
                                updateField("checklist", cl);
                              }}
                            ...
                            <div className="relative bg-zinc-900/20 border border-zinc-800/40 group-hover/subcomment:border-zinc-800 rounded-2xl p-4">
                              ...
                              <div className="absolute top-2 right-2 opacity-0 group-hover/subcomment:opacity-100 flex gap-1">
                                <button
                                  onClick={() =>
                                    setEditingSubtaskCommentIndex(i)
                                  }
                                ...
                                <button
                                  onClick={() => {
                                    const cl = [...payload.checklist];
                                    cl[selectedSubtaskIndex].comments = cl[
            ...
            <div className="px-8 py-4 border-t border-zinc-900 bg-zinc-900/50 flex justify-end shrink-0">
              <button
                onClick={() => {
                  const cl = [...payload.checklist];
                  cl[selectedSubtaskIndex].completed =
                    !cl[selectedSubtaskIndex].completed;
                  updateField("checklist", cl);
                }}
                className={cn(
