...
export default function CalculatorsAdminView() {
  ...
  const enabledCategoryCount = CALCULATOR_CATEGORIES.filter(
    (category) => normalized.enabledCategories[category.id] !== false,
  ).length;
  const enabledCalculatorCount = CALCULATOR_DEFINITIONS.filter(
    (calculator) => normalized.enabledCalculators[calculator.id] !== false,
  ).length;
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div>
              ...
              <p className="text-zinc-400 mt-1">
                Configure category and calculator visibility for your public
                calculators.
              </p>
            </div>
        ...
        <div className="xl:col-span-1 rounded-2xl border border-zinc-800 bg-zinc-900 p-5 space-y-4">
          ...
          <div className="space-y-2">
            ...
            {CALCULATOR_CATEGORIES.map((category) => {
              const enabled =
                normalized.enabledCategories[category.id] !== false;
                ...
                >
                  <div>
                    <p
                      className={cn(
                        "text-sm font-medium",
                        enabled ? "text-accent" : "text-zinc-300",
                      )}
                    >
                      {category.name}
                    </p>
        ...
        <div className="xl:col-span-2 rounded-2xl border border-zinc-800 bg-zinc-900 p-5 space-y-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <p className="text-xs text-zinc-500">
              Same component as public route (live)
            </p>
          </div>
