    ...
    const filtered = useMemo(() => {
        ...
        return [...items]
            .filter((item) => {
                ...
                const haystack = `${item.payload.title} ${item.payload.source_domain || ""} ${item.payload.type} ${item.payload.notes || ""}`.toLowerCase();
            ...
            {showForm && (
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 animate-fade-in-up">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg font-semibold text-zinc-50">Add to Reading Queue</h2>
