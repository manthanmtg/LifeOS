...
export default function BookshelfAdminView() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: Record<string, unknown> = {
      ...
      started_at:
        toISODate(startedAt) ||
        (status === "reading" && !editingId
          ? new Date().toISOString()
          : undefined),
      finished_at:
        toISODate(finishedAt) ||
        (status === "completed" && !editingId
          ? new Date().toISOString()
          : undefined),
      tags: tagsInput
        .split(",")
        ...
        <div className="relative space-y-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="text-zinc-400 mt-1">
                Track reading momentum, completion velocity, and yearly goals in
                one dashboard.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/50 px-3 py-2.5">
              ...
              <p className="text-lg font-semibold text-zinc-50">
                {stats.goal > 0 ? stats.goal : "Not set"}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                onChange={(event) =>
                  updateSettings({ defaultStatus: event.target.value })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  updateSettings({
                    yearlyGoal: Number.parseInt(event.target.value, 10) || 0,
                  })
                }
              ...
              <p className="text-xs text-zinc-500 mt-1">
                Used to track completed books against annual target.
              </p>
            </div>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                      <span
                        className={cn(
                          ...
                          STATUS_STYLES[book.payload.status],
                        )}
                      >
