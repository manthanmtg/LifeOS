...
export function AnalyticsTab({
  ...
  const activeCrop = useMemo(
    () => crops?.find((c: CropConfig) => c.id === activeCropId),
    [crops, activeCropId],
  );
  ...
  const records = useMemo(
    () =>
      (allRecords || []).filter(
        (r: CropRecord) => r.payload.crop_id === activeCropId,
      ),
  ...
  const schedulePeriods = useMemo(() => {
    ...
    const periods = Array.from(
      new Set(records.map((r: CropRecord) => r.payload.schedule_period)),
    );
    return periods.sort();
  }, [records]);
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center py-12 shadow-sm">
        ...
        <p className="text-zinc-500 mt-1">
          Configure crops and areas, then add data in the Spreadsheet tab to see
          analytics.
        </p>
      </div>
    );
        ...
        {crops.map((c: CropConfig) => (
          <button
            ...
            className={cn(
              ...
              c.id === activeCropId
                ? "bg-success/15 text-success border border-success/30 shadow-inner"
...
function CropAnalytics({
  ...
  const recordsByPeriod = useMemo(() => {
    ...
    records.forEach((r: CropRecord) => {
      map[r.payload.schedule_period] = r.payload;
    });
    return map;
  }, [records]);

  // Resolve which fields to use for analytics
  const revenueFieldId =
    activeCrop.analyticsConfig?.revenueFieldId ||
    activeCrop.calculatedFields.find(
      (f: CalcFieldDef) => f.format === "currency",
    )?.id;
  const yieldFieldId =
    activeCrop.analyticsConfig?.yieldFieldId || activeCrop.sourceFields[0]?.id;

  const revenueField = activeCrop.calculatedFields.find(
    (f: CalcFieldDef) => f.id === revenueFieldId,
  );
  const yieldField = activeCrop.sourceFields.find(
    (f: FieldDef) => f.id === yieldFieldId,
  );
  ...
  const contextByPeriod = useMemo(() => {
    ...
    schedulePeriods.forEach((period: string) => {
      ...
      const calcResults = evaluateAllCalculatedFields(
        activeCrop.calculatedFields,
        ctx,
      );
  ...
  const kpiData = useMemo(() => {
    const periods = schedulePeriods.filter(
      (p: string) => Object.keys(contextByPeriod[p] || {}).length > 0,
    );
    ...
    const prevRevenue =
      revenueField && prevCtx ? prevCtx[revenueField.id] || 0 : 0;
    ...
    periods.forEach((p: string) => {
      totalRevenue += revenueField
        ? contextByPeriod[p]?.[revenueField.id] || 0
        : 0;
    });
    ...
    return {
      ...
      revenue: {
        ...
        avg: periods.length ? totalRevenue / periods.length : 0,
        label: revenueField?.name || "Revenue",
      },
      yield: {
        current: latestYield,
        previous: prevYield,
        label: yieldField?.name || "Yield",
        unit: yieldField?.unit,
      },
      bestArea: {
        name: bestArea?.name || "-",
        value: bestAreaVal,
        fieldName: yieldField?.name || "",
        unit: yieldField?.unit,
      },
  ...
  const revenueTrendData = useMemo(() => {
    ...
    return schedulePeriods.map((period: string) => ({
      name: period,
      revenue: contextByPeriod[period]?.[revenueField.id] || 0,
    }));
  ...
  const trendData = useMemo(() => {
    return schedulePeriods.map((period: string) => {
      ...
      activeCrop.sourceFields.forEach((f: FieldDef) => {
        entry[f.name] = ctx[f.id] || 0;
      });
      return entry;
    });
  ...
  const noData =
    !kpiData || schedulePeriods.every((p: string) => !recordsByPeriod[p]);
  ...
  if (noData) {
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center py-12 shadow-sm">
        ...
        <p className="text-zinc-500 mt-1">
          Add data in the Spreadsheet tab for <strong>{activeCrop.name}</strong>{" "}
          to see analytics.
        </p>
      </div>
    );
  ...
  return (
    <>
      ...
      {perAreaChartData.length > 0 && (
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
          <h3 className="text-lg font-medium text-zinc-200 mb-1">
            {activeCrop.name} by Area
          </h3>
          <p className="text-xs text-zinc-500 mb-6">
            {yieldField?.name || "Primary field"} per area, grouped by period
          </p>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={perAreaChartData}
              ...
              >
                ...
                <YAxis
                  stroke="#52525b"
                  fontSize={12}
                  tickFormatter={(val: number) => formatNum(val, 0)}
                />
                <Tooltip
                  cursor={{ fill: "#1a1a1e" }}
                  contentStyle={{
      ...
      {revenueTrendData.length > 0 &&
        revenueTrendData.some((d: { revenue: number }) => d.revenue > 0) && (
          <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
            <h3 className="text-lg font-medium text-zinc-200 mb-1">
              Revenue Trend
            </h3>
            <p className="text-xs text-zinc-500 mb-6">
              {revenueField?.name || "Income"} over time
            </p>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <defs>
                    <linearGradient
                      id={`revenueGradient-${activeCrop.id}`}
                      x1="0"
                      y1="0"
                      x2="0"
                      y2="1"
                  ...
                  <Area
                    ...
                    dataKey="revenue"
                    stroke="#34d399"
                    fill={`url(#revenueGradient-${activeCrop.id})`}
                    strokeWidth={2}
                    name={revenueField?.name || "Revenue"}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
      ...
      {trendData.length > 0 && activeCrop.sourceFields.length > 0 && (
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
          ...
          <p className="text-xs text-zinc-500 mb-6">
            Total per-area field values over time
          </p>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
        ...
        <p className="text-xs text-zinc-500 mb-4">
          All metrics with change indicators
        </p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {activeCrop.sourceFields.map((f: FieldDef) => {
                const values = schedulePeriods.map(
                  (p: string) => contextByPeriod[p]?.[f.id] || 0,
                );
                const last2 = values.slice(-2);
                const change =
                  last2.length === 2 && last2[0] > 0
                    ? ((last2[1] - last2[0]) / last2[0]) * 100
                    : null;
                return (
                  <tr key={f.id} className="hover:bg-zinc-800/30">
                    ...
                    {values.map((v: number, i: number) => (
                      <td
                        key={i}
                        className="text-right px-3 py-2.5 text-zinc-200 font-mono"
                      >
                        {formatNum(v, 1)}
                      </td>
                    ))}
