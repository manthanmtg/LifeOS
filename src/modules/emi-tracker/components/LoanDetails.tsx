...
export default function LoanDetails({
  ...
  const schedule = useMemo(() => computeSchedule(loan.payload, decimals), [loan.payload, decimals]);
  ...
  const handleUpdatePayments = async (payments: EmiLoan["payload"]["payments"]) => {
    await onUpdate({ ...loan.payload, payments });
  };

  const handleUpdateDocs = async (documents: EmiLoan["payload"]["documents"]) => {
    await onUpdate({ ...loan.payload, documents });
  };

  const handleUpdateAdjustments = async (rate_adjustments: EmiLoan["payload"]["rate_adjustments"]) => {
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/80 rounded-3xl p-6 shadow-xl">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h2 className="text-2xl font-bold text-zinc-50 tracking-tight">{loan.payload.title}</h2>
          ...
          >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 shadow-lg space-y-6">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 mt-1">Visualize how extra monthly payments impact your loan tenure and interest.</p>
                    </div>

                    <div className="space-y-4">
                      <div className="flex items-center justify-between text-xs font-bold px-1">
                        <span className="text-zinc-400 uppercase tracking-widest">Extra Monthly Payment</span>
                        <span className="text-accent text-sm font-black tabular-nums">{formatMoney(extraMonthly, sym, 0, numberFormat)}</span>
                      </div>
                      <input 
                        type="range"
                        min="0"
            ...
            {activeTab === "schedule" && (
              <ScheduleTable 
                ...
                onExportCSV={() => {
                  ...
                  downloadTextFile(`${loan.payload.title}_schedule.csv`, csv, "text/csv");
                }}
                onPrintPDF={() => {
                  exportSchedulePDF(loan.payload.title, schedule, sym, decimals, numberFormat);
                }}
              />
            )}
            ...
            {activeTab === "adjustments" && (
              <RateAdjustmentList 
                ...
                isSubmitting={isSubmitting}
                onAdd={async (a) => {
                  await handleUpdateAdjustments([...loan.payload.rate_adjustments, a]);
                }}
                onDelete={async (idx) => {
