...
export default function BlogView() {
    ...
    fetch("/api/content?module_type=blog_post")
      ...
      .then((d) => {
        const published = (d.data || []).filter(
          (p: BlogPost) => p.payload.status === "published",
        ) as BlogPost[];
        setPosts(
          published.sort(
            (a, b) =>
  ...
  const totalMinutes = posts.reduce(
    (total, item) =>
      total +
      (item.payload.estimated_reading_time ||
        estimateReadingTime(item.payload.content)),
    0,
  );
      ...
      <div className="max-w-6xl mx-auto">
        <section className="relative overflow-hidden border border-zinc-800 rounded-3xl bg-zinc-900/70 p-6 md:p-8 mb-8">
          ...
          <div className="relative">
            ...
            <h1 className="text-3xl md:text-5xl font-semibold tracking-tight text-zinc-100">
              Long-form ideas, guides, and build notes.
            </h1>
            <p className="text-zinc-400 mt-3 max-w-2xl">
              A reading-focused archive with practical writeups and experiments
              from Life OS.
            </p>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3 md:max-w-xl">
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/80 px-3 py-2.5">
                <p className="text-lg md:text-xl font-semibold text-zinc-100">
                  {posts.length}
                </p>
                  ...
                  >
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-4 text-xs text-zinc-400">
                      ...
                      {featured.payload.tags.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className="px-2 py-0.5 rounded-full bg-zinc-800/80 border border-zinc-700 text-zinc-300"
                        >
                          {tag}
                        </span>
                      ))}
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        <div className="flex flex-wrap gap-1.5">
                          {post.payload.tags.slice(0, 2).map((tag) => (
                            <span
                              key={tag}
                              className="text-[11px] px-2 py-0.5 rounded-full bg-zinc-800 border border-zinc-700 text-zinc-400"
                            >
                              {tag}
                            </span>
                          ))}
