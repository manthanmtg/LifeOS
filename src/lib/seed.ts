...
export async function ensureSystemConfig() {
  try {
    ...
    if (!exists) {
      const defaultModuleRegistry: Record<
        string,
      ...
      if (appModules) {
        Object.entries(appModules).forEach(([key, mod]) => {
          defaultModuleRegistry[key] = {
            enabled: true,
            isPublic: (mod as { defaultPublic: boolean }).defaultPublic,
          };
        });
      }
      ...
      if (Object.keys(updates).length > 0) {
        ...
        await systemColl.updateOne(
          { _id: "global" },
          { $set: { moduleRegistry: merged } },
        );
        console.log(
          `[Seed] Backfilled ${Object.keys(updates).length} new module(s).`,
        );
      }
    }
