...
Find and fix any **currently failing tests** in the LifeOS codebase. This is a repair-only prompt — you are not adding new tests, just making the existing ones pass again.
...
- If **all tests pass**, you're done — no-op. Report "all tests passing" in the run summary and stop; do not create an `issues_to_look/` entry for a healthy test suite.
