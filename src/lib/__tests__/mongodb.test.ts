...
vi.mock('mongodb', () => {
  ...
  return {
    MongoClient: vi.fn().mockImplementation(function(this: any) { return mClient; }),
...
describe('mongodb.ts', () => {
  ...
  it('throws a standardized error if connection fails in getDb', async () => {
    ...
    (MongoClient as any).mockImplementationOnce(function(this: any) { return mClient; });

    const { getDb } = await import('../mongodb');
    await expect(getDb()).rejects.toThrow('Database service is currently unavailable.');
  });
});
