...
export default function SnippetsAdminView() {
  ...
  const stats = useMemo(() => {
    ...
    const favorites = snippets.filter(
      (snippet) => snippet.payload.is_favorite,
    ).length;
    const languages = new Set(
      snippets.map((snippet) => snippet.payload.language),
    ).size;
    const averageLength =
      snippets.length > 0
        ? Math.round(
            snippets.reduce(
              (sum, snippet) => sum + snippet.payload.code.split("\n").length,
              0,
            ) / snippets.length,
          )
        : 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                onChange={(event) =>
                  updateSettings({ defaultLanguage: event.target.value })
                }
              ...
              <input
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter") {
                    ...
                    if (
                      normalized &&
                      !configuredLanguages.includes(normalized)
                    ) {
                      updateSettings({
                        languages: [...configuredLanguages, normalized],
                      });
                      setNewLang("");
                    }
                  }
            ...
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                  <button
                    onClick={() =>
                      handleCopy(snippet._id, snippet.payload.code)
                    }
                    className={cn(
                  ...
                  >
                    {isProcessingId === snippet._id ? (
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    ) : (
                      <Star
                        className="w-3.5 h-3.5"
                        fill={
                          snippet.payload.is_favorite ? "currentColor" : "none"
                        }
                      />
                    )}
                  </button>
