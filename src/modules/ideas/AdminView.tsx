      ...
      <div className="flex items-center gap-1.5 flex-wrap mb-1 ml-4">
        <span
          className={cn(
            ...
            PRIORITY_STYLES[idea.payload.priority] || PRIORITY_STYLES.medium,
          )}
        >
        ...
        <h3 className="text-[11px] font-bold text-zinc-500 uppercase tracking-[0.1em] flex items-center gap-2">
          <span
            className={cn(
              ...
              STATUS_STYLES[id]?.split(" ")[0] || "bg-zinc-500",
            )}
          />
          {title}
        </h3>
...
export default function IdeasAdminView() {
  ...
  const stats = useMemo(() => {
    ...
    const active = ideas.filter((idea) =>
      ["raw", "exploring"].includes(idea.payload.status),
    ).length;
