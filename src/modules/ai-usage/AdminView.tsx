...
import { AiUsageLogTab } from "./AiUsageLogTab";
  ...
  anthropic: {
    ...
    plans: [
      ...
      { name: "Build Tier 2", rateLimit: "1,000 RPM", monthlyLimit: "$500/mo" },
      {
        name: "Build Tier 3",
        rateLimit: "2,000 RPM",
        monthlyLimit: "$1,000/mo",
      },
      {
        name: "Build Tier 4",
        rateLimit: "4,000 RPM",
        monthlyLimit: "$5,000/mo",
      },
...
const UNSUPPORTED_PROVIDERS: Record<
  string,
  {
    name: string;
    color: string;
...
export function fmtCost(cost: number, currency = "USD"): string {
...
export default function AiUsageAdminView() {
  ...
  const [limitsMap, setLimitsMap] = useState<
    Record<string, ProviderLimitResult>
  >({});
  ...
  useEffect(() => {
    Promise.all([fetchEntries(), fetchProviders(), fetchLimits()]).finally(() =>
      setLoading(false),
    );
  ...
  const handleSaveProvider = async () => {
    if (
      ...
      (!editingProvider && !providerForm.admin_api_key.trim())
    ) {
    ...
    try {
      ...
      if (providerForm.admin_api_key.trim())
  ...
  const stats = useMemo(() => {
    const totalCost = dateFilteredEntries.reduce(
      (s, e) => s + e.payload.cost,
      0,
    );
    const totalIn = dateFilteredEntries.reduce(
      (s, e) => s + e.payload.input_tokens,
      0,
    );
    const totalOut = dateFilteredEntries.reduce(
      (s, e) => s + e.payload.output_tokens,
      0,
    );
    const totalCacheR = dateFilteredEntries.reduce(
      (s, e) => s + (e.payload.cache_read_tokens || 0),
      0,
    );
    const totalCacheW = dateFilteredEntries.reduce(
      (s, e) => s + (e.payload.cache_write_tokens || 0),
      0,
    );
    const totalReq = dateFilteredEntries.reduce(
      (s, e) => s + (e.payload.num_requests || 0),
      0,
    );
    ...
    const byModel: Record<
      string,
      { cost: number; calls: number; tokens: number; provider: string }
    > = {};
    ...
    for (const e of dateFilteredEntries) {
      ...
      if (!byModel[m])
        byModel[m] = { cost: 0, calls: 0, tokens: 0, provider: p };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {hasProviders && (
            <button
              onClick={() => handleSync()}
              disabled={syncing || activeProviders.length === 0}
              className={cn(
                ...
                {r.status === "success" ? (
                  r.entries_synced > 0 ? (
                    <span className="text-success/80">
                      {r.entries_synced} entries synced
                    </span>
                  ) : (
                    <span className="text-warning/80">
                      0 entries — this tracks{" "}
                      <strong>API key usage only</strong> (not Claude.ai /
                      ChatGPT web sessions). Use your Admin key from{" "}
                      <a
                        href={
                      ...
                      and make sure API calls have been made with keys in that
                      same org.
                    </span>
                  )
                ) : (
                  <span className="text-danger/80 truncate max-w-xs">
                    {r.error}
                  </span>
                )}
              </div>
        ...
        {(
          [
            { id: "dashboard", label: "Dashboard", icon: BarChart3 },
            {
              ...
              label: "Providers",
              icon: Shield,
              badge: providers.length || undefined,
            },
          ...
          <button
            ...
            className={cn(
              ...
              tab === id
                ? "bg-zinc-800 text-zinc-50 shadow-lg shadow-zinc-900/50"
      ...
      <AnimatePresence mode="wait">
              ...
              <>
                ...
                >
                  {providers.map((p) => {
                    ...
                    const pStats = providerStats[p._id] || {
                      cost: 0,
                      tokens: 0,
                      requests: 0,
                    };
                    const budgetPct =
                      p.monthly_budget && p.monthly_budget > 0
                        ? Math.min((pStats.cost / p.monthly_budget) * 100, 100)
                        : null;
                    return (
                      <div
                        key={p._id}
                        ...
                        <div className="relative p-5 space-y-4">
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-lg font-bold text-zinc-200 mt-0.5">
                                {pStats.requests.toLocaleString()}
                              </p>
                            </div>
                          </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <StatCard
                    ...
                    sub={
                      stats.totalCacheR > 0
                        ? `${fmtTokens(stats.totalCacheR)} cached`
                        : undefined
                    }
                    icon={Hash}
                    accent="text-warning"
                  ...
                  <StatCard
                    ...
                    sub={
                      stats.totalCacheW > 0
                        ? `${fmtTokens(stats.totalCacheW)} cache writes`
                        : undefined
                    }
                    icon={TrendingUp}
                    accent="text-accent"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="p-5 rounded-2xl border border-zinc-800/50 bg-zinc-900/30 space-y-4">
                    ...
                    {Object.entries(stats.byProvider)
                      ...
                      .map(([prov, data]) => {
                        const m = PROVIDER_META[prov as SupportedProvider] || {
                          name: prov,
                          color: "text-zinc-400",
                          colorHex: "#a1a1aa",
                          bg: "bg-zinc-500/10",
                    ...
                    {Object.keys(stats.byProvider).length === 0 && (
                      <EmptyChart text="No usage data yet. Hit 'Sync All' to fetch." />
                    )}
                  </div>
                  ...
                  <div className="p-5 rounded-2xl border border-zinc-800/50 bg-zinc-900/30 space-y-3">
                    ...
                    {Object.entries(stats.byModel)
                      ...
                      .map(([model, data], i) => {
                        const m = PROVIDER_META[
                          data.provider as SupportedProvider
                        ] || {
                          name: data.provider,
                          color: "text-zinc-400",
                          colorHex: "#a1a1aa",
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="text-sm font-bold text-zinc-300 flex-shrink-0">
                              {fmtCost(data.cost)}
                            </p>
                          </div>
                        );
                    ...
                    {Object.keys(stats.byModel).length === 0 && (
                      <EmptyChart text="No model data yet." />
                    )}
                  </div>
                </div>
                  ...
                  <div className="p-5 rounded-2xl border border-zinc-800/50 bg-zinc-900/30 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-[10px] text-zinc-600 font-bold">
                        ...
                        {fmtCost(
                          stats.totalCost /
                            Math.max(Object.keys(stats.dailyCosts).length, 1),
                        )}
                        /day
                      </p>
                    </div>
                    <div className="flex items-end gap-[3px] h-28">
                      {(() => {
                        const sorted = Object.entries(stats.dailyCosts)
          ...
          >
            <div className="p-4 rounded-xl border border-zinc-800/50 bg-zinc-900/30 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 mt-1">
                  Add your provider&apos;s{" "}
                  <strong className="text-zinc-300">Admin API key</strong> and
                    ...
                    <div className="relative">
                      ...
                      <div className="relative p-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-1 text-xs text-zinc-500">
                              ...
                              {p.monthly_budget ? (
                                <span className="text-zinc-600">
                                  · Budget: {fmtCost(p.monthly_budget)}/mo
                                </span>
                              ) : null}
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            onClick={() =>
                              handleToggleProvider(p._id, p.is_active)
                            }
            ...
            {!showAddProvider ? (
              ...
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="p-6 rounded-2xl border border-zinc-800/50 bg-zinc-900/30 space-y-5"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="text"
                      value={providerForm.organization_name}
                      onChange={(e) =>
                        setProviderForm((f) => ({
                          ...f,
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setProviderForm((f) => ({ ...f, plan: e.target.value }))
                      }
                    ...
                    >
                      ...
                      {PROVIDER_META[providerForm.provider].plans.map((pl) => (
                        <option key={pl.name} value={pl.name}>
                          {pl.name} — {pl.rateLimit}
                          {pl.monthlyLimit ? ` (${pl.monthlyLimit})` : ""}
                        </option>
                      ))}
                    </select>
                  </div>
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      min={0}
                      step="1"
                      value={providerForm.monthly_budget || ""}
                      onChange={(e) =>
                        setProviderForm((f) => ({
                          ...f,
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider flex items-center gap-2">
                    {PROVIDER_META[providerForm.provider].keyLabel}{" "}
                    {!editingProvider && "*"}
                    <a
                      href={PROVIDER_META[providerForm.provider].apiKeyUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={providerForm.admin_api_key}
                      onChange={(e) =>
                        setProviderForm((f) => ({
                          ...f,
                ...
                <div className="p-3 rounded-xl bg-zinc-950/80 border border-zinc-800/50 flex items-start gap-2">
                  ...
                  <p className="text-xs text-zinc-500">
                    This must be an{" "}
                    <strong className="text-zinc-300">Admin API key</strong>,
                    not a regular API key.
                    {providerForm.provider === "openai"
                      ? " Only Organization Owners can create admin keys."
                      : " Only Organization Admins can create admin keys."}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  <button
                    ...
                    className="h-10 px-5 rounded-xl border border-zinc-800 text-zinc-400 text-sm font-medium hover:border-zinc-700 hover:text-zinc-300 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className={cn(
                      "h-10 px-6 rounded-xl text-sm font-bold transition-all flex items-center gap-2",
                      savingProvider || !providerForm.name.trim()
        ...
        {/* ─── USAGE LOG ──────────────────────────────────────────── */}
        {tab === "log" && <AiUsageLogTab entries={dateFilteredEntries} />}
          ...
          >
            <div className="p-4 rounded-xl border border-zinc-800/50 bg-zinc-900/30 flex items-start gap-3">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-zinc-500">
                  Step-by-step instructions to get Admin API keys for automatic
                  usage tracking.
                </p>
                <div className="p-3 rounded-lg bg-warning/5 border border-warning/20">
                  <p className="text-xs text-warning font-semibold mb-1">
                    What this tracks vs. what it does not
                  </p>
                  <p className="text-xs text-zinc-400 leading-relaxed">
                    ...
                    — calls made with{" "}
                    <code className="text-accent">sk-ant-...</code> /{" "}
                    <code className="text-accent">sk-...</code> keys from code,
                    Claude Code CLI, or apps. It does{" "}
                    <span className="text-danger font-medium">not</span> track
                    Claude.ai / ChatGPT web or app sessions — those are separate
                    consumer products with no public usage API.
                  </p>
                  <p className="text-xs text-zinc-500 mt-2">
                    If you use{" "}
                    <strong className="text-zinc-300">Claude Code</strong>: your
                    usage appears here automatically once you configure the
                    Admin key from the same org as your Claude Code API key.
                  </p>
                </div>
              </div>
            </div>
              ...
              return (
                <div
                  ...
                  className={cn(
                    "rounded-2xl border transition-all overflow-hidden",
                    isExpanded
                      ? cn(config.border, "bg-zinc-900/50")
                      : "border-zinc-800/50 bg-zinc-900/20 hover:border-zinc-700/50",
                  )}
                >
                  <button
                    onClick={() =>
                      setExpandedGuide(isExpanded ? null : provider)
                    }
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-zinc-500">
                          {config.description} — Requires Admin API key
                        </p>
                      </div>
                    </div>
                    {isExpanded ? (
                        ...
                        <div className="px-4 pb-5 space-y-5">
                              ...
                              <table className="w-full text-xs">
                                <thead>
                                  <tr className="bg-zinc-800/30">
                                    <th className="text-left px-3 py-2 text-zinc-500 font-bold uppercase tracking-wider">
                                      Plan
                                    </th>
                                    <th className="text-left px-3 py-2 text-zinc-500 font-bold uppercase tracking-wider">
                                      Rate Limit
                          ...
                          <div className="flex gap-3 flex-wrap">
                            <a
                              ...
                              rel="noopener noreferrer"
                              className={cn(
                                "flex items-center gap-2 px-4 py-2 rounded-xl border text-xs font-semibold transition-all hover:opacity-80",
                                config.border,
                                config.color,
                                config.bg,
                              )}
                            ...
                            >
                              <Zap className="w-3.5 h-3.5" /> Get Admin Key{" "}
                              <ExternalLink className="w-3 h-3" />
                            </a>
                            <a
                              href={config.usageDashboardUrl}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="flex items-center gap-2 px-4 py-2 rounded-xl border border-zinc-800 text-xs font-semibold text-zinc-400 hover:border-zinc-700 hover:text-zinc-300 transition-all"
                            >
                          ...
                          <div className="p-3 rounded-xl bg-zinc-950/80 border border-zinc-800/50 flex items-center justify-between">
                            ...
                            <button
                              onClick={() => copyToClipboard(config.envVar)}
                              className="p-2 rounded-lg hover:bg-zinc-800 text-zinc-600 hover:text-zinc-400 transition-colors"
                            >
                              {copiedText === config.envVar ? (
    ...
    <div className="text-center py-20 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-2 max-w-md mx-auto leading-relaxed">
          Add your OpenAI or Anthropic Admin API key to automatically track
          usage, costs, token consumption, and model breakdowns — all in one
          place.
        </p>
      </div>
