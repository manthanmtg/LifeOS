...
export default function CalculatorsPublicView({
  items: _items,
  settingsOverride,
}: CalculatorsPublicViewProps) {
  void _items;
  ...
  const visibleCategories = useMemo(
    () =>
      CALCULATOR_CATEGORIES.filter(
        (category) => normalized.enabledCategories[category.id] !== false,
      ),
  ...
  const visibleCalculators = useMemo(() => {
    return CALCULATOR_DEFINITIONS.filter((calculator) => {
      const categoryEnabled =
        normalized.enabledCategories[calculator.categoryId] !== false;
      const calculatorEnabled =
        normalized.enabledCalculators[calculator.id] !== false;
      if (!categoryEnabled || !calculatorEnabled) return false;

      if (activeCategory !== "all" && calculator.categoryId !== activeCategory)
        return false;
      ...
      const haystack =
        `${calculator.name} ${calculator.description} ${calculator.shortName}`.toLowerCase();
  ...
  const activeCategoryMeta = useMemo(() => {
    ...
    return (
      CALCULATOR_CATEGORIES.find(
        (category) => category.id === activeCalculator.categoryId,
      ) || null
    );
  }, [activeCalculator]);
      ...
      <div className="relative overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900/80 p-6 sm:p-8">
        ...
        <div className="relative">
          ...
          <p className="text-zinc-400 mt-3 max-w-3xl text-sm sm:text-base">
            Browse available tools with a clean catalog. Click any calculator to
            open it in a focused modal.
          </p>
        </div>
      </div>
