...
export default function RecurringExpensesAdminView() {
  ...
  const fetchSubs = useCallback(async () => {
    try {
      ...
      switch (sortBy) {
        ...
        case "name-desc":
          sorted = [...unsorted].sort((a, b) =>
            b.payload.name.localeCompare(a.payload.name),
          );
          break;
        case "cost-asc":
          sorted = [...unsorted].sort(
            (a, b) => a.payload.cost - b.payload.cost,
          );
          break;
        case "cost-desc":
          sorted = [...unsorted].sort(
            (a, b) => b.payload.cost - a.payload.cost,
          );
          break;
        case "monthly-eq-asc":
          sorted = [...unsorted].sort(
            (a, b) =>
              monthlyEquivalent(a.payload.cost, a.payload.billing_cycle) -
              monthlyEquivalent(b.payload.cost, b.payload.billing_cycle),
          );
          break;
        ...
        case "category":
          sorted = [...unsorted].sort((a, b) => {
            const catCompare = a.payload.category.localeCompare(
              b.payload.category,
            );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(costVal) || costVal <= 0) {
      setFormError("Valid cost required");
      return;
    }
  ...
  const activeDragItem = activeDragId
    ? (subs.find((s) => s._id === activeDragId) ?? null)
    : null;
  const dropAnimation = {
    duration: 220,
  ...
  const visibleSubs = useMemo(() => {
    ...
    return subs.filter((sub) => {
      ...
      if (
        ...
        (days < 0 || days > settings.renewalWarningDays)
      )
        return false;
        ...
        <div className="mb-4 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="text-zinc-400 mt-1">
              Track renewals, burn rate, and urgency in one control center.
            </p>
          </div>
          <div className="flex items-center gap-2 md:pt-1 md:shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-accent hover:bg-accent-hover text-zinc-50 font-medium px-4 py-2.5 rounded-xl text-sm transition-colors whitespace-nowrap"
            >
      ...
      {showSettings && (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 animate-fade-in-up space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label
                htmlFor="settings-number-format"
                className="block text-xs text-zinc-500 mb-1.5"
              >
                Number Format
              </label>
            ...
            <div>
              ...
              <input
                ...
                min={1}
                max={30}
                value={settings.renewalWarningDays}
                onChange={(e) =>
                  updateSettings({
                    renewalWarningDays: parseInt(e.target.value) || 7,
                  })
                }
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-zinc-50 focus:outline-none focus:ring-2 focus:ring-accent/40"
              />
              <p className="text-xs text-zinc-500 mt-1">
                Show warning when renewal is within this many days
              </p>
            </div>
            <div className="flex items-center gap-3 pt-6">
              <label className="flex items-center gap-2 text-sm text-zinc-400 cursor-pointer">
                <input
                  id="settings-default-reminders"
                  type="checkbox"
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    ...
                    if (newCat.trim()) {
                      ...
                      updateSettings({
                        categories: [...settings.categories, trimmedCat],
                      });
      ...
      {showForm && (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 animate-fade-in-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-zinc-50">
              {editingId ? "Edit" : "New"} Recurring Expense
            </h2>
            <button
              onClick={resetForm}
          ...
          >
            ...
            <div>
              <label
                htmlFor="expense-next-renewal"
                className="block text-xs text-zinc-500 mb-1.5"
              >
                Next Renewal
              </label>
              <input
                id="expense-next-renewal"
                type="date"
                value={nextRenewal}
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {(
                [
                  ...
                  {
                    key: "warning",
                    label: `Due ≤ ${settings.renewalWarningDays}d`,
                  },
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              >
                ...
                <option value="monthly-eq-desc">
                  Monthly Eq. (High to Low)
                </option>
                <option value="monthly-eq-asc">
                  Monthly Eq. (Low to High)
                </option>
      ...
      {subs.length === 0 ? (
        ...
        >
          ...
          <DragOverlay dropAnimation={dropAnimation} adjustScale={false}>
            {activeDragItem ? (
              <DragPreviewCard s={activeDragItem} sym={sym} />
            ) : null}
          </DragOverlay>
        </DndContext>
      )}
