...
- Identify and pick one **autonomous-safe prompt** at random by running the following shell command. `prompts_optimizer.md` should run rarely, about 1 in 25 runs, because it maintains the prompt suite itself. This run also avoids prompts with no-op-heavy behavior, where `totalNoop` already exceeds `totalCompleted`, so selection stays action-oriented:
```bash
node -e 'const fs=require("fs"); const isOptimizer = Math.floor(Math.random() * 25) === 0; if (isOptimizer) { const optimizerPrompt="prompts/prompts_optimizer.md"; if (fs.existsSync(optimizerPrompt)) { console.log(optimizerPrompt); process.exit(0); } console.error("prompts_optimizer prompt file missing; selecting normal prompt pool instead."); } const metadata=JSON.parse(fs.readFileSync("prompts/prompts_metadata.json","utf8")); const candidates=Object.values(metadata.prompts).filter((prompt)=>prompt.enabled&&prompt.autonomousSafe&&prompt.file!=="prompts_optimizer.md"&&(prompt.totalNoop||0)<=prompt.totalCompleted).map((prompt)=>`prompts/${prompt.file}`).filter((candidate)=>fs.existsSync(candidate)).sort(); if(candidates.length===0){console.error("No eligible prompts found in prompts/prompts_metadata.json."); process.exit(1);} console.log(candidates[Math.floor(Math.random()*candidates.length)]);'
```
