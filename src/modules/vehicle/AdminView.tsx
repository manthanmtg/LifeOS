...
export default function VehicleAdminView() {
  ...
  const updateVehiclePayload = async (
    vehicle: Vehicle,
    newPayload: VehiclePayload,
  ) => {
    try {
  ...
  const saveServiceRecord = async () => {
    ...
    await updateVehiclePayload(selectedVehicle, {
      ...selectedVehicle.payload,
      service_records: records,
    });
    showToast(
      editingService ? "Service record updated" : "Service record added",
    );
  ...
  const deleteServiceRecord = async (id: string) => {
    ...
    const records = selectedVehicle.payload.service_records.filter(
      (r) => r.id !== id,
    );
    await updateVehiclePayload(selectedVehicle, {
      ...selectedVehicle.payload,
  ...
  const selectedStats = useMemo(() => {
    ...
    const totalServiceCost = (p.service_records || []).reduce(
      (sum, r) => sum + (r.cost || 0),
      0,
    );
    const totalFuelCost = (p.fuel_logs || []).reduce(
      (sum, l) => sum + l.cost,
      0,
    );

    // Calculate fuel efficiency from full tank fills
    const sortedLogs = [...(p.fuel_logs || [])]
      .filter((l) => l.odometer)
    ...
    for (let i = 1; i < sortedLogs.length; i++) {
      if (sortedLogs[i].full_tank && sortedLogs[i - 1].full_tank) {
        const dist =
          (sortedLogs[i].odometer || 0) - (sortedLogs[i - 1].odometer || 0);
      ...
      <div className="animate-fade-in-up space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold text-zinc-50 tracking-tight truncate">
              {p.name}
            </h1>
        ...
        <div className="flex gap-1 bg-zinc-900/50 p-1 rounded-xl border border-zinc-800">
          {(["overview", "service", "fuel", "documents"] as DetailTab[]).map(
            (tab) => {
              const icons = {
                overview: Gauge,
                service: Wrench,
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
                ...
                <p className="text-2xl font-bold text-zinc-50">
                  {p.odometer_reading.toLocaleString()}
                </p>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn("text-lg font-bold", ftConfig.color)}>
                    {ftConfig.label}
                  </span>
                </div>
              </div>
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
                ...
                <p className="text-2xl font-bold text-zinc-50">
                  {selectedStats?.totalServiceCost.toLocaleString()}
                </p>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
                ...
                <p className="text-2xl font-bold text-zinc-50">
                  {selectedStats?.totalFuelCost.toLocaleString()}
                </p>
                <p className="text-xs text-zinc-500">total spent</p>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-success">
                      {selectedStats.avgEfficiency.toFixed(1)}{" "}
                      <span className="text-sm text-zinc-500">
                        {selectedStats.efficiencyUnit}
                      </span>
                    </p>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { label: "Insurance", date: p.insurance_expiry, icon: Shield },
                {
                  ...
                  date: p.pollution_certificate_expiry,
                  icon: Wind,
                },
                {
                  label: "Next Service",
                  date: p.next_service_due,
                  icon: Wrench,
                },
              ].map((item) => {
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  {p.next_service_odometer && (
                    <div>
                      ...
                      <p className="text-zinc-200 font-medium">
                        {p.next_service_odometer.toLocaleString()}{" "}
                        {p.odometer_unit}
                      </p>
                    </div>
                  )}
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-sm text-zinc-400">
                {p.service_records.length} record
                {p.service_records.length !== 1 ? "s" : ""}
              </p>
              <button
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3 min-w-0">
                            ...
                            >
                              <StIcon
                                className={cn("w-4 h-4", stConfig.color)}
                              />
                            </div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={() => openServiceForm(record)}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 focus:outline-none focus:border-zinc-600"
                            />
                          </div>
                          <div>
                            <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest block mb-1.5">
                            ...
                            <select
                              value={serviceForm.type || "routine"}
                              onChange={(e) =>
                                setServiceForm((f) => ({
                                  ...f,
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              placeholder={`in ${p.odometer_unit}`}
                              className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder:text-zinc-700 focus:outline-none focus:border-zinc-600"
                            />
                          </div>
                        </div>
        ...
        {activeTab === "fuel" && (
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-sm text-zinc-400">
                {p.fuel_logs.length} entr
                {p.fuel_logs.length !== 1 ? "ies" : "y"}
                {selectedStats?.avgEfficiency && (
                  <span className="ml-2 text-success font-semibold">
                    Avg: {selectedStats.avgEfficiency.toFixed(1)}{" "}
                    {selectedStats.efficiencyUnit}
                  </span>
                )}
              </p>
            ...
            {p.fuel_logs.length === 0 ? (
              <div className="bg-zinc-900 border border-dashed border-zinc-800 rounded-2xl p-12 text-center">
                ...
                <p className="text-xs text-zinc-600 mt-1">
                  Log your fuel fills to track mileage and costs
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <div className="flex gap-2">
                              <input
                                ...
                                placeholder="0"
                                className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder:text-zinc-700 focus:outline-none focus:border-zinc-600"
                              />
                              <select
                                value={fuelForm.fuel_unit || "liters"}
                                onChange={(e) =>
                                  setFuelForm((f) => ({
                                    ...f,
                                    fuel_unit: e.target.value as FuelUnit,
                                  }))
                                }
                                className="bg-zinc-900 border border-zinc-800 rounded-xl px-2 py-2.5 text-sm text-zinc-200 focus:outline-none focus:border-zinc-600"
                              >
          ...
          >
            ...
            {p.documents.length === 0 ? (
              <div className="bg-zinc-900 border border-dashed border-zinc-800 rounded-2xl p-12 text-center">
                ...
                <p className="text-xs text-zinc-600 mt-1">
                  Add insurance, registration, and other documents
                </p>
              </div>
            ) : (
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="relative h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${status === "expired" ? 100 : Math.max(5, Math.min(100, ((365 - (days || 0)) / 365) * 100))}%`,
                              }}
                            />
                          </div>
                      ...
                      <div className="p-6 space-y-4 flex-1 min-h-0 overflow-y-auto">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="e.g., ICICI Lombard Motor Insurance"
                            className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder:text-zinc-700 focus:outline-none focus:border-zinc-600"
                          />
                        </div>
                        <div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400">
            Track your vehicles, services, fuel, and documents.
          </p>
        </div>
        <button
      ...
      {vehicles.length === 0 ? (
        <div className="bg-zinc-900 border border-dashed border-zinc-800 rounded-2xl p-16 text-center">
          ...
          <p className="text-sm text-zinc-600 mb-6 max-w-md mx-auto">
            Add your first vehicle to start tracking service history, fuel
            consumption, and document expiry dates.
          </p>
          <button
            onClick={openAddVehicle}
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  hasAlerts
                    ? "border-warning/20 hover:border-warning/40 hover:shadow-warning/5"
                    : "border-zinc-800 hover:border-zinc-700 hover:shadow-zinc-800/30",
                )}
                onClick={() => {
              ...
              >
                ...
                <div className="flex items-center gap-1 text-xs text-zinc-500 mb-4">
                  ...
                  <span className="font-medium text-zinc-300">
                    {vp.odometer_reading.toLocaleString()}
                  </span>
                ...
                <div className="p-6 space-y-5 flex-1 min-h-0 overflow-y-auto">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., My Creta, Dad's Activa"
                      className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder:text-zinc-700 focus:outline-none focus:border-zinc-600"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 focus:outline-none focus:border-zinc-600"
                      >
                        {Object.entries(FUEL_TYPE_CONFIG).map(([k, v]) => (
                          <option key={k} value={k}>
                            {v.label}
                          </option>
                        ))}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData((f) => ({
                            ...f,
                            odometer_reading: Number(e.target.value) || 0,
                          }))
                        }
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 focus:outline-none focus:border-zinc-600"
                      />
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setFormData((f) => ({
                            ...f,
                            odometer_unit: e.target.value as OdometerUnit,
                          }))
                        }
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 focus:outline-none focus:border-zinc-600"
                      >
                  ...
                  <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <input
                          type="date"
                          value={formatDateInput(formData.insurance_expiry)}
                          onChange={(e) =>
                            setFormData((f) => ({
                              ...f,
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={formatDateInput(
                            formData.pollution_certificate_expiry,
                          )}
                          onChange={(e) =>
                            setFormData((f) => ({
                              ...f,
                              pollution_certificate_expiry: e.target.value
                                ? toISODate(e.target.value)
                                : undefined,
                            }))
                          }
                      ...
                      <div>
                        ...
                        <input
                          type="date"
                          value={formatDateInput(formData.next_service_due)}
                          onChange={(e) =>
                            setFormData((f) => ({
                              ...f,
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={`e.g., 15000 ${formData.odometer_unit}`}
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-sm text-zinc-200 placeholder:text-zinc-700 focus:outline-none focus:border-zinc-600"
                      />
                    </div>
                  </div>
