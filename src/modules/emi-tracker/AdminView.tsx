...
function computeSchedule(
  ...
  if (
    loan.interest_type === "floating" &&
    strategy === "keep_emi_adjust_tenure" &&
    rows.length >= hardCapMonths
  ) {
...
export default function EmiTrackerAdminView() {
  ...
  } = useModuleSettings<EmiTrackerSettings>("emiTrackerSettings", DEFAULTS);
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "schedule" | "payments" | "documents" | "rates"
  >("overview");
  ...
  const [recastStrategy, setRecastStrategy] = useState<RecastStrategy>(
    settings.defaultRecastStrategy,
  );
  ...
  const selectedProgress = useMemo(() => {
    ...
    const basePrincipal =
      selected.payload.principal +
      (selected.payload.processing_fee_financed
        ? (selected.payload.processing_fee_amount ?? 0) +
          (selected.payload.processing_fee_percent
            ? (selected.payload.processing_fee_percent / 100) *
              selected.payload.principal
            : 0)
        : 0);
    const outstanding = selectedOutstanding?.outstanding ?? basePrincipal;
    const cleared =
      basePrincipal > 0 ? (basePrincipal - outstanding) / basePrincipal : 0;
    ...
    const clearedTotal =
      totalLifetime > 0 ? Math.min(1, paid / totalLifetime) : 0;

    return {
      clearedPrincipalPct: Math.max(0, Math.min(1, cleared)),
      clearedTotalPct: Math.max(0, clearedTotal),
      basePrincipal,
      totalLifetime,
      paid,
    };
  ...
  const openEdit = (loan: EmiLoan) => {
    ...
    setProcessingFeeAmount(
      loan.payload.processing_fee_amount !== undefined
        ? String(loan.payload.processing_fee_amount)
        : "",
    );
    setProcessingFeePercent(
      loan.payload.processing_fee_percent !== undefined
        ? String(loan.payload.processing_fee_percent)
        : "",
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const feeAmt = processingFeeAmount
      ? parseFloat(processingFeeAmount)
      : undefined;
    const feePct = processingFeePercent
      ? parseFloat(processingFeePercent)
      : undefined;

    if (!title.trim()) return setFormError("Loan title is required");
    if (!Number.isFinite(p) || p <= 0)
      return setFormError("Valid loan amount is required");
    if (!Number.isFinite(n) || n <= 0)
      return setFormError("Valid tenure (months) is required");
    if (!Number.isFinite(r) || r < 0)
      return setFormError("Valid interest rate is required");
    if (!Number.isFinite(emi) || emi <= 0)
      return setFormError("Valid monthly EMI amount is required");
    if (!category.trim()) return setFormError("Category is required");
    if (!currency || currency.length !== 3)
      return setFormError("Currency must be a 3-letter code");
    if (!Number.isFinite(dueDay) || dueDay < 1 || dueDay > 28)
      return setFormError("Due day must be between 1 and 28");
    if (!startDate) return setFormError("Start date is required");

    const existingPayload = editingId
      ? loans.find((l) => l._id === editingId)?.payload
      : null;
    ...
    const payload: EmiLoan["payload"] = {
      ...
      processing_fee_amount:
        feeAmt !== undefined && Number.isFinite(feeAmt)
          ? roundTo(Math.max(0, feeAmt), decimals)
          : undefined,
      processing_fee_percent:
        feePct !== undefined && Number.isFinite(feePct)
          ? Math.max(0, feePct)
          : undefined,
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      alert(
        "File is quite large (>5MB). It may increase database size significantly. Proceeding anyway...",
      );
    }
  ...
  const updateSelectedPayload = async (
    updates: Partial<EmiLoan["payload"]>,
  ) => {
    if (!selected) return;
    setIsSubmittingAction(true);
    try {
  ...
  const exportScheduleCSV = () => {
    ...
    downloadTextFile(
      `emi-schedule-${selected.payload.title.replace(/\s+/g, "-").toLowerCase()}.csv`,
      toCSV(rows),
      "text/csv",
    );
  };
  ...
  const exportPaymentsCSV = () => {
    ...
    downloadTextFile(
      `emi-payments-${selected.payload.title.replace(/\s+/g, "-").toLowerCase()}.csv`,
      toCSV(rows),
      "text/csv",
    );
  };
  ...
  const printReport = () => {
    ...
    const pdfSym =
      selected.payload.currency === "INR"
        ? "Rs."
        : selected.payload.currency === "EUR"
          ? "EUR"
          : selected.payload.currency === "GBP"
            ? "GBP"
            : selectedSym;
    ...
    drawMeta(
      "Interest Rate",
      `${selected.payload.annual_interest_rate}% p.a.`,
      160,
    );
    ...
    drawMeta(
      "Total Principal",
      formatMoney(
        selectedSchedule.totals.total_principal,
        pdfSym,
        decimals,
        settings.numberFormat,
      ),
    ...
    drawMeta(
      "Total Interest",
      formatMoney(
        selectedSchedule.totals.total_interest,
        pdfSym,
        decimals,
        settings.numberFormat,
      ),
    ...
    const tableData = selectedSchedule.rows.map((r) => [
      ...
      r.prepayment > 0
        ? formatMoney(r.prepayment, "", decimals, settings.numberFormat)
        : "-",
  ...
  const quickStats = useMemo(() => {
    ...
    for (const l of active) {
      ...
      outstandingByCurrencyMap[currencyKey] =
        (outstandingByCurrencyMap[currencyKey] || 0) + out.outstanding;
      if (out.nextDue) {
        if (
          !nearestDue ||
          new Date(out.nextDue.due_date).getTime() <
            new Date(nearestDue.row.due_date).getTime()
        ) {
  ...
  const loanCards = useMemo(() => {
    ...
    return loans
      .map((l) => {
        ...
        const pct =
          basePrincipal > 0
            ? (basePrincipal - out.outstanding) / basePrincipal
            : 0;
        return {
          loan: l,
      ...
      .sort((a, b) => {
        const ad = a.nextDue
          ? new Date(a.nextDue.due_date).getTime()
          : Number.POSITIVE_INFINITY;
        const bd = b.nextDue
          ? new Date(b.nextDue.due_date).getTime()
          : Number.POSITIVE_INFINITY;
        return ad - bd;
      });
  }, [loans, decimals]);
  ...
  const selectedSym = selected
    ? CURR_SYM[selected.payload.currency] || selected.payload.currency
    : sym;
  ...
  const computedEmiHint = useMemo(() => {
    ...
    const r = parseFloat(annualRate);
    const fee =
      (processingFeeAmount ? parseFloat(processingFeeAmount) : 0) +
      (processingFeePercent
        ? (parseFloat(processingFeePercent) / 100) *
          (Number.isFinite(p) ? p : 0)
        : 0);
    const base = Number.isFinite(p)
      ? p + (processingFeeFinanced ? (Number.isFinite(fee) ? fee : 0) : 0)
      : 0;
    if (
        ...
        <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between relative z-10">
          <div>
            ...
            <p className="text-zinc-400 mt-1">
              Track loans, schedules, and repayment progress.
            </p>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 bg-zinc-950/40 p-5 rounded-xl border border-zinc-800/50">
            <div>
              ...
              <p className="text-[11px] text-zinc-500 mt-2 font-medium">
                Used for new loans; each loan can override.
              </p>
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">
                Default Floating Recast Strategy
              </label>
              <select
                value={settings.defaultRecastStrategy}
                onChange={(e) =>
                  updateSettings({
                    defaultRecastStrategy: e.target.value as RecastStrategy,
                  })
                }
                className="w-full bg-zinc-900/80 border border-zinc-700/80 rounded-xl px-4 py-3 text-sm text-zinc-50 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 transition-all shadow-sm"
              >
        ...
        <div className="bg-zinc-900/60 backdrop-blur-xl border border-zinc-800/80 rounded-2xl p-6 shadow-xl animate-fade-in-up relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="text-zinc-400 hover:text-white bg-zinc-800/50 hover:bg-zinc-700/80 border border-zinc-700/50 rounded-xl p-2 transition-all shadow-sm"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
              ...
              <div>
                ...
                {computedEmiHint !== null && (
                  <p className="text-[11px] text-zinc-500 mt-2 flex items-center gap-1">
                    <Calculator className="w-3 h-3" /> Formula EMI ≈{" "}
                    <span className="text-zinc-300">
                      {formatMoney(
                        computedEmiHint,
                        CURR_SYM[currency] || currency,
                        decimals,
                        settings.numberFormat,
                      )}
                    </span>{" "}
                    (your EMI is used for the schedule)
                  </p>
                )}
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <label className="flex items-center gap-2.5 bg-zinc-900/80 border border-zinc-700/80 rounded-xl px-4 py-3 text-sm font-medium text-zinc-300 hover:bg-zinc-800 transition-colors cursor-pointer shadow-sm">
                    <input
                      type="checkbox"
                      checked={processingFeeFinanced}
                      onChange={(e) =>
                        setProcessingFeeFinanced(e.target.checked)
                      }
                      className="accent-accent w-4 h-4 rounded border-zinc-700 bg-zinc-900/50"
                    />
                    Financed
                  </label>
                </div>
                <p className="text-[11px] text-zinc-500 mt-2 font-medium">
                  If financed, fee is added to principal for schedule.
                </p>
              </div>
              {interestType === "floating" && (
                <div className="md:col-span-2">
                  <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-2">
                    Floating Recast Strategy
                  </label>
                  <select
                    value={recastStrategy}
                    onChange={(e) =>
                      setRecastStrategy(e.target.value as RecastStrategy)
                    }
                    className="w-full bg-zinc-900/80 border border-zinc-700/80 rounded-xl px-4 py-3 text-sm text-zinc-50 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50 transition-all shadow-sm"
                  >
                    <option value="keep_tenure_adjust_emi">
              ...
              loanCards.map(({ loan, outstanding, nextDue, progress }) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedId === loan._id
                      ? "bg-zinc-800/60 border-accent/80 shadow-[0_0_15px_-3px_rgba(var(--color-accent),0.15)] ring-1 ring-accent/50"
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 mt-1">
                        {loan.payload.lender_name
                          ? `${loan.payload.lender_name} · `
                          : ""}
                        {loan.payload.category} ·{" "}
                        {loan.payload.interest_type === "floating"
                          ? "Floating"
                          : "Fixed"}
                      </p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-base font-bold text-zinc-100">
                        {formatMoney(
                          outstanding,
                          CURR_SYM[loan.payload.currency] ||
                            loan.payload.currency,
                          decimals,
                          settings.numberFormat,
                        )}
                      </p>
                      <p className="text-[10px] font-medium uppercase tracking-wider text-zinc-500 mt-1">
                        {nextDue ? `due ${nextDue.due_date.slice(0, 10)}` : "—"}
                      </p>
                    </div>
                  </div>
                  <div className="mt-4">
              ...
              <div className="p-5 md:p-6 border-b border-zinc-800/80 bg-zinc-950/40 relative overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-4 mb-3 relative z-10 w-full">
                  <div className="flex items-center gap-2 shrink-0">
                    <span
                      className={cn(
                        ...
                        selected.payload.status === "active"
                          ? "bg-success shadow-success/50"
                          : "bg-zinc-500",
                      )}
                    />
                    <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest leading-none">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      title="Edit Loan"
                      className="p-2.5 md:px-4 md:py-2.5 rounded-xl bg-zinc-800/80 border border-zinc-700/50 text-zinc-300 hover:bg-zinc-700 hover:text-white transition-all shadow-sm"
                    >
                ...
                <div className="relative z-10">
                  ...
                  <div className="text-sm text-zinc-400 mt-2 flex flex-wrap items-center gap-x-2 gap-y-1.5 leading-snug break-words max-w-full">
                    ...
                    <span className="bg-zinc-800/80 px-2 py-0.5 rounded text-xs shrink-0">
                      {selected.payload.category}
                    </span>
                    <span className="w-1 h-1 rounded-full bg-zinc-700 shrink-0" />
                    <span className="text-accent/80 font-medium shrink-0">
                      {selected.payload.annual_interest_rate}%
                    </span>
                    <span className="w-1 h-1 rounded-full bg-zinc-700 shrink-0" />
                    <span className="text-zinc-100 font-semibold text-lg shrink-0">
                      {formatMoney(
                        selected.payload.monthly_emi,
                        selectedSym,
                        decimals,
              ...
              <div className="p-5 space-y-5">
                {selectedSchedule?.warnings?.length ? (
                  <div className="bg-warning/10 border border-warning/20 text-warning rounded-xl px-4 py-3 text-sm">
                    {selectedSchedule.warnings.map((w, i) => (
                      <div key={i}>{w}</div>
                    ))}
                  </div>
                ) : null}
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-zinc-950/40 backdrop-blur-md border border-zinc-800/60 rounded-xl p-5 relative overflow-hidden group">
                          ...
                          <p className="text-xs font-medium text-zinc-500 mb-1 tracking-wide uppercase">
                            Outstanding Balance
                          </p>
                          <p className="text-3xl font-bold text-zinc-50 mt-1 whitespace-nowrap">
                            {formatMoney(
                        ...
                        <div className="bg-zinc-950/40 backdrop-blur-md border border-zinc-800/60 rounded-xl p-5 relative overflow-hidden group">
                          ...
                          <p className="text-3xl font-bold text-zinc-50 mt-1 whitespace-nowrap">
                            {selectedOutstanding.nextDue
                              ? selectedOutstanding.nextDue.due_date.slice(
                                  0,
                                  10,
                                )
                              : "—"}
                          ...
                          <p className="text-[11px] text-zinc-500 mt-2 font-medium">
                            {selectedOutstanding.nextDue
                              ? `Month ${selectedOutstanding.nextDue.index} / ${selected.payload.tenure_months}`
                              : ""}
                          </p>
                        </div>
                        <div className="bg-zinc-950/40 backdrop-blur-md border border-zinc-800/60 rounded-xl p-5 relative overflow-hidden group">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-zinc-950/40 backdrop-blur-md border border-zinc-800/60 rounded-xl p-6 flex flex-col justify-center">
                          <div className="flex items-end justify-between mb-4">
                            <div>
                              ...
                              <p className="text-[11px] text-zinc-400 mt-0.5">
                                Principal cleared over scheduled timeline
                              </p>
                            </div>
                            <span className="text-2xl font-bold text-accent">
                              {(
                                selectedProgress.clearedPrincipalPct * 100
                              ).toFixed(0)}
                              %
                            </span>
                          </div>
                          <div className="h-3 rounded-full bg-zinc-900 border border-zinc-800 relative overflow-hidden shadow-inner">
                            <div
                              className="absolute top-0 bottom-0 left-0 bg-accent rounded-full transition-all duration-1000 ease-out"
                              style={{
                          ...
                          <div className="mt-4 pt-4 border-t border-zinc-800/50 grid grid-cols-2 gap-y-3 gap-x-4 text-[11px] text-zinc-400">
                            <div>
                              <p className="uppercase tracking-wider text-zinc-500 mb-1">
                                Principal Paid
                              </p>
                              <p className="font-medium text-success">
                                {formatMoney(
                                  selectedProgress.basePrincipal -
                                    selectedOutstanding.outstanding,
                                  selectedSym,
                                  decimals,
                                  settings.numberFormat,
                        ...
                        <div className="bg-zinc-950/40 backdrop-blur-md border border-zinc-800/60 rounded-xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 flex-wrap">
                          <div className="flex-1 min-w-[240px]">
                            ...
                            <p className="text-[11px] text-zinc-400 mt-0.5">
                              Lifetime breakdown based on current schedule
                            </p>
                            ...
                            <div className="mt-4 space-y-2">
                              <div className="flex items-center justify-between p-2 gap-3 rounded-lg bg-zinc-900/60 border border-zinc-800/50 shadow-sm">
                                <div className="flex items-center gap-2 shrink-0">
                                  <div className="w-2.5 h-2.5 rounded bg-zinc-700/50 flex items-center justify-center">
                                ...
                                <span className="text-sm font-semibold text-zinc-100 text-right truncate">
                                  {formatMoney(
                                    selectedSchedule.totals.total_interest,
                                    selectedSym,
                                    decimals,
                              ...
                              <div className="flex items-center justify-between p-2 gap-3 rounded-lg bg-zinc-900/60 border border-zinc-800/50 shadow-sm">
                                <div className="flex items-center gap-2 shrink-0">
                                  <div className="w-2.5 h-2.5 rounded bg-zinc-700/50 flex items-center justify-center">
                                    <div className="w-1.5 h-1.5 rounded-full bg-success shadow-[0_0_8px_rgba(var(--success),0.6)]" />
                                  </div>
                                  <span className="text-xs font-medium text-zinc-300">
                                    Principal
                                  </span>
                            ...
                            <div className="mt-6 space-y-2">
                              ...
                              <div className="h-3 rounded-full bg-zinc-900 border border-zinc-800 flex overflow-hidden shadow-inner">
                                <div
                                  ...
                                  style={{
                                    width: `${((selectedSchedule.totals.total_interest / (selectedSchedule.totals.total_interest + selectedSchedule.totals.total_principal)) * 100 || 0).toFixed(0)}%`,
                                  }}
                                />
                                <div
                                  className="h-full bg-success transition-all duration-1000 ease-out"
                                  style={{
                                    width: `${((selectedSchedule.totals.total_principal / (selectedSchedule.totals.total_interest + selectedSchedule.totals.total_principal)) * 100 || 0).toFixed(0)}%`,
                                  }}
                                />
                              </div>
                            </div>
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <div className="text-xs text-zinc-500 flex items-center gap-2">
                        ...
                        {selectedSchedule.computed_emi_suggestion !== null && (
                          <span className="ml-2">
                            ...
                            <span className="text-zinc-300">
                              {formatMoney(
                                selectedSchedule.computed_emi_suggestion,
                                selectedSym,
                                decimals,
                                settings.numberFormat,
                              )}
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-3 py-2 rounded-lg bg-zinc-800 text-zinc-300 hover:bg-zinc-700 text-sm flex items-center gap-2"
                        >
                        ...
                        <table className="min-w-[900px] w-full text-xs">
                          ...
                          <tbody className="divide-y divide-zinc-800/50 bg-zinc-900/40">
                            {selectedSchedule.rows.map((r) => (
                              ...
                              >
                                <td className="px-5 py-3 text-left font-medium text-zinc-500 group-hover:text-zinc-400">
                                  {r.index}
                                </td>
                                <td className="px-5 py-3 text-left text-zinc-300">
                                  {r.due_date.slice(0, 10)}
                                </td>
                                ...
                                <td className="px-5 py-3 text-right text-rose-400/90 font-medium">
                                  {formatMoney(
                                    r.interest,
                                    selectedSym,
                                    decimals,
                      ...
                      <div className="p-5 border-t border-zinc-800/80 grid grid-cols-2 md:grid-cols-4 gap-4 bg-zinc-950/50">
                        <div className="bg-zinc-900 rounded-lg p-3 border border-zinc-800">
                          ...
                          <p className="text-zinc-300 font-semibold mt-1">
                            {formatMoney(
                              selectedSchedule.totals.total_emi,
                              selectedSym,
                              decimals,
                              settings.numberFormat,
                            )}
                        ...
                        <div className="bg-zinc-900 rounded-lg p-3 border border-zinc-800">
                          ...
                          <p className="text-zinc-300 font-semibold mt-1">
                            {formatMoney(
                              selectedSchedule.totals.total_prepayment,
                              selectedSym,
                              decimals,
                              settings.numberFormat,
                            )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                        ...
                        <div>
                          ...
                          <select
                            value={payKind}
                            onChange={(e) =>
                              setPayKind(e.target.value as PaymentKind)
                            }
                            className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-50 focus:outline-none focus:ring-2 focus:ring-accent/40"
                          >
                      ...
                      {(selected.payload.payments || []).length === 0 ? (
                        ...
                        <div className="overflow-auto">
                          <table className="min-w-[720px] w-full text-xs">
                            ...
                            <tbody className="divide-y divide-zinc-800/50 bg-zinc-900/40">
                              {(selected.payload.payments || []).map(
                                (p, idx) => (
                                  <tr
                                    key={`${p.date}-${idx}`}
                                    className="hover:bg-accent/5 transition-colors group"
                                  >
                                    <td className="px-5 py-3.5 text-left text-zinc-300 font-medium">
                                      {p.date.slice(0, 10)}
                                    </td>
                                    ...
                                    <td className="px-5 py-3.5 text-right font-medium text-zinc-100">
                                      {formatMoney(
                                        p.amount,
                                        selectedSym,
                                        decimals,
                                    ...
                                    <td className="px-5 py-3.5 text-left text-zinc-400">
                                      {p.note || "—"}
                                    </td>
                                    <td className="px-5 py-3.5 text-left">
                                      {p.receipt_url ? (
                    ...
                    <div className="bg-zinc-950/40 backdrop-blur-md border border-zinc-800/60 rounded-xl p-5 shadow-sm">
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-50 focus:outline-none focus:ring-2 focus:ring-accent/40"
                          >
                            <option value="sanction_letter">
                              Sanction Letter
                            </option>
                            <option value="noc">NOC</option>
                            <option value="interest_certificate">
                            ...
                            <div className="flex items-center justify-between p-3 rounded-xl border border-accent/20 bg-accent/5">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-[10px] text-zinc-500 line-clamp-1 break-all">
                              ...
                              <div className="relative group cursor-pointer">
                                ...
                                <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-zinc-700/60 rounded-xl bg-zinc-900/30 group-hover:bg-zinc-800/40 group-hover:border-accent/40 transition-all text-center">
                                  ...
                                  <p className="text-[11px] text-zinc-500 mt-1">
                                    PDF, PNG, JPG accepted. Saved directly to
                                    database.
                                  </p>
                                  {isUploadingDoc && (
                                    <p className="text-[11px] text-accent mt-2 animate-pulse">
                                      Processing file...
                                    </p>
                                  )}
                                </div>
                      ...
                      <div className="mt-4">
                        <button
                          onClick={addDocument}
                          disabled={
                            !docTitle.trim() || !docUrl.trim() || isUploadingDoc
                          }
                      ...
                      {(selected.payload.documents || []).length === 0 ? (
                        ...
                        <div className="overflow-auto">
                          <table className="min-w-[760px] w-full text-xs">
                            ...
                            <tbody className="divide-y divide-zinc-800/50 bg-zinc-900/40">
                              {(selected.payload.documents || []).map(
                                (d, idx) => (
                                  <tr
                                    key={`${d.added_at}-${idx}`}
                                    className="hover:bg-accent/5 transition-colors group"
                                  >
                                    <td className="px-5 py-3.5 text-left text-zinc-400 capitalize">
                                      {d.type.replace(/_/g, " ")}
                                    </td>
                                    <td className="px-5 py-3.5 text-left font-medium text-zinc-200">
                                      {d.title}
                                    </td>
                                    <td className="px-5 py-3.5 text-left text-zinc-400">
                                      {d.issued_at
                ...
                {activeTab === "rates" && selected && (
                  <div className="space-y-4">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            updateSelectedPayload({
                              recast_strategy: e.target.value as RecastStrategy,
                            })
                          }
                    ...
                    <div className="bg-zinc-950/40 backdrop-blur-md border border-zinc-800/60 rounded-xl shadow-sm overflow-hidden">
                      {(selected.payload.rate_adjustments || []).length ===
                      0 ? (
                        <div className="p-5 text-sm text-zinc-500">
                        ...
                        <div className="overflow-auto">
                          <table className="min-w-[720px] w-full text-xs">
                            ...
                            <tbody className="divide-y divide-zinc-800/50 bg-zinc-900/40">
                              {(selected.payload.rate_adjustments || []).map(
                                (a, idx) => (
                                  <tr
                                    key={`${a.effective_date}-${idx}`}
                                    className="hover:bg-accent/5 transition-colors group"
                                  >
                                    <td className="px-5 py-3.5 text-left text-zinc-300 font-medium">
                                      {a.effective_date.slice(0, 10)}
                                    </td>
                                    <td className="px-5 py-3.5 text-right font-mono text-zinc-200">
                                      {a.annual_interest_rate.toFixed(2)}
                                    </td>
                                    <td className="px-5 py-3.5 text-left text-zinc-400">
                                      {a.note || "—"}
                                    </td>
