...
- **Contract Fit**: Does the widget already use one of the approved `WidgetStat`/`WidgetHighlight`/`WidgetMiniStats`/`WidgetList` layouts and the `/api/widgets/summary` endpoint?
...
- Respect the Widget Contract: keep exactly one hero metric plus at most one detail element, use the approved `Widget*` primitives, fetch compact data from `/api/widgets/summary`, and avoid internal `<button>`, `<input>`, form, or standalone link elements. The whole card is the action.
...
- If improving the widget would require changing the API, backend summary shape, or a broad contract refactor, log it to `issues_to_look/` instead of making the change.
