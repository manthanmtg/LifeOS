...
export default function LoanList({
    ...
    return (
      <div className="space-y-3" aria-label="Loading loans">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="w-full rounded-lg border border-zinc-800 bg-zinc-900/50 p-4"
          >
  ...
  if (loanCards.length === 0) {
    return (
      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-7 text-center">
        ...
        {onEmptyAction && (
          <button
            ...
            className={cn(
              "mt-5 min-h-[44px] rounded-md bg-accent px-5 py-2 text-sm font-black text-zinc-50 hover:bg-accent-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/70",
              PRESSABLE,
            )}
          >
            {emptyActionLabel}
